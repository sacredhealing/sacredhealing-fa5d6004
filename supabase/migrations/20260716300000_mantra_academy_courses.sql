-- Mantra Academy: real Supabase-backed tables, matching the established
-- pattern. Metadata here, rich content (typed lesson sections --
-- intro/teaching/mantra/practice/wisdom/audio) in
-- src/data/mantraAcademyData.ts. 24 modules -- genuinely large curriculum.
-- Progress is tracked per LESSON (lesson.id is already globally unique),
-- not per module, since that's the real unit of completion here.
...
CREATE INDEX IF NOT EXISTS idx_user_mantra_academy_lesson_progress_user
  ON public.user_mantra_academy_lesson_progress(user_id);
