...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.palm_oracle_courses TO authenticated;
GRANT ALL ON public.palm_oracle_courses TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.user_palm_oracle_progress TO authenticated;
GRANT ALL ON public.user_palm_oracle_progress TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.user_palm_oracle_section_progress TO authenticated;
GRANT ALL ON public.user_palm_oracle_section_progress TO service_role;
...
CREATE OR REPLACE FUNCTION public.update_user_palm_oracle_section_progress_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
DROP TRIGGER IF EXISTS update_user_palm_oracle_section_progress_updated_at
  ON public.user_palm_oracle_section_progress;

CREATE TRIGGER update_user_palm_oracle_section_progress_updated_at
  BEFORE UPDATE ON public.user_palm_oracle_section_progress
  FOR EACH ROW EXECUTE FUNCTION public.update_user_palm_oracle_section_progress_updated_at();
