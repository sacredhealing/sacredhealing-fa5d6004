...
BEGIN
  ...
  WHERE dt.published = true AND dt.price_usd IS NOT NULL AND dt.created_at >= now() - interval '8 days'
  ...
  IF p_kind = 'mantra' OR p_kind IS NULL THEN
    SELECT 'mantra' AS kind, m.title, (m.duration_seconds / 60)::text || ' min' AS duration_label, '/mantras' AS url_path
    INTO v FROM public.mantras m WHERE m.is_active = true ORDER BY random() LIMIT 1;
    IF v.title IS NOT NULL THEN RETURN QUERY SELECT v.kind, v.title, v.duration_label, v.url_path; RETURN; END IF;
  END IF;
  ...
  IF p_kind = 'meditation' OR p_kind IS NULL THEN
    ...
    INTO v FROM public.meditations me WHERE (p_category IS NULL OR me.category = p_category) ORDER BY random() LIMIT 1;
    IF v.title IS NOT NULL THEN RETURN QUERY SELECT v.kind, v.title, v.duration_label, v.url_path; RETURN; END IF;
  END IF;
  ...
  IF p_kind = 'music' OR p_kind IS NULL THEN
    ...
    INTO v FROM public.music_tracks mt ORDER BY random() LIMIT 1;
    IF v.title IS NOT NULL THEN RETURN QUERY SELECT v.kind, v.title, v.duration_label, v.url_path; RETURN; END IF;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.get_featured_content(TEXT, TEXT) TO authenticated, anon, service_role;