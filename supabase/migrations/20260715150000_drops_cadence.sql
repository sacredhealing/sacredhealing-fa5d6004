-- "Drops" cadence, built on existing infrastructure rather than a new
-- system: get_featured_content (already used in lakshmi-friday) now
-- checks for a recently-published, individually-priced Divine
-- Transmission FIRST, before falling back to the existing random mantra/
-- meditation/music pick. This makes new paid content actually get
-- announced through the email cadence you already have, without building
-- a separate "drops" page or notification system.
--
-- "Recent" = published in the last 8 days, matching the weekly Friday
-- cadence — a drop stays featured for about one send cycle before the
-- picker falls back to evergreen content again.
...
DECLARE
  v RECORD;
BEGIN
  -- New: check for a live "drop" first, regardless of p_kind — a real
  -- priced transmission published in the last 8 days beats generic content.
