-- Yogananda Codex: real Supabase-backed tables, matching the established
-- pattern. Metadata here, rich chapter content (free/prana/siddha/akasha
-- layers, tier-gated secrets, activation practice) in
-- src/data/yoganandaCodexContent.ts. 49 chapters, each individually tier-gated.
...
CREATE INDEX IF NOT EXISTS idx_user_yogananda_codex_progress_user ON public.user_yogananda_codex_progress(user_id);
