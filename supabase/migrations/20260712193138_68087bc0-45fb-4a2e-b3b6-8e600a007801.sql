...
GRANT SELECT ON public.email_teachings TO authenticated;
GRANT ALL ON public.email_teachings TO service_role;
...
GRANT ALL ON public.user_teaching_log TO service_role;
...
DROP POLICY IF EXISTS "Anyone authenticated can read active teachings" ON public.email_teachings;
CREATE POLICY "Anyone authenticated can read active teachings" ON public.email_teachings
...
BEGIN
  ...
  WHERE t.is_active = true
    AND (p_theme IS NULL OR t.theme = p_theme)
  GROUP BY t.id, t.theme, t.title, t.body_text, t.source_note
