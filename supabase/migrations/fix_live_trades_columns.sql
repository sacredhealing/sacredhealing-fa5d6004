...
ALTER TABLE shreem_brzee_live_trades ADD COLUMN IF NOT EXISTS opened_at TIMESTAMPTZ DEFAULT NOW();
...
ALTER TABLE shreem_brzee_live_trades DROP CONSTRAINT IF EXISTS shreem_brzee_live_trades_status_check;
ALTER TABLE shreem_brzee_live_trades ADD CONSTRAINT shreem_brzee_live_trades_status_check CHECK (status IN ('pending','open','closed','unconfirmed','failed'));
