...
CREATE UNIQUE INDEX IF NOT EXISTS temple_activations_user_active_idx
  ON public.temple_activations (user_id) WHERE is_active = true;

CREATE INDEX IF NOT EXISTS temple_activations_active_idx
  ON public.temple_activations (is_active, last_pulse_at) WHERE is_active = true;
...
CREATE POLICY "Users can view own activation"
  ON public.temple_activations FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own activation"
  ON public.temple_activations FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own activation"
  ON public.temple_activations FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Service role can pulse all activations"
  ON public.temple_activations FOR UPDATE USING (auth.role() = 'service_role');
