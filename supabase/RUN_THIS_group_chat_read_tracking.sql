...
-- Run this once in the Supabase SQL Editor (Lovable-locked project ssygukfdbtehvtndandn)
-- Same pattern as supabase/QUICK_FIX_FOR_MEMBERS.sql — paste and run.
-- ============================================================================

-- One row per (user, room): when did this user last read this room's messages.
...
-- Total unread group-message count across every room the user belongs to.
-- SECURITY DEFINER so it can read chat_messages/chat_members efficiently in one round trip.
...
-- Mark a room as read up to now — called when a user opens a circle/room.
