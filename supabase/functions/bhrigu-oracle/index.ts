...
${chartAnalysis ? `\n${chartAnalysis}\n\nUSE THIS DATA for any domain-specific question — not just the dasha lord. Identify which house(s) govern the domain being asked about in the D1 chart (examples: 3rd = short journeys/courage, 9th = long journeys/dharma/fortune, 12th = foreign travel/loss/liberation, 7th = partnership/marriage, 10th = career, 2nd/11th = wealth), then read: the planet(s) placed there, their dignity (exalted/debilitated/own/moolatrikona changes the strength of a placement significantly), whether they are combust, what conjoins them, and what aspects (drishti) reach that house from elsewhere in the chart. Then cross-check against the divisional charts provided, each with its own Lagna and houses: NAVAMSA (D9) for marriage, dharma, and the true inner strength of any D1 placement — D9 frequently overrides what D1 alone suggests, especially for relationship and long-term-outcome questions; DASAMSA (D10) for career and public-life questions; SAPTAMSA (D7) for questions about children. SHASHTIAMSA (D60) is included as a supplementary fine layer only — its own note already flags that it's highly sensitive to birth-time precision, so use it to add texture or resolve a genuine tie between other signals, never as your primary or sole basis for an answer. A single well-placed or well-aspected planet can override an otherwise weak house, a strong D1 house can be undone by an afflicting aspect or debilitation, and a difficult D1 placement can be redeemed by a strong divisional-chart placement or vice versa — read the full layered picture across all charts provided, not one data point in isolation, and say so explicitly when different layers point in different directions. Answer from what this data actually shows. Do not default to a favorable answer because it is what the seeker wants to hear, and do not invent a relationship the data doesn't support.` : `\nNo full chart-analysis data was provided for this reading. You do NOT have the individual planetary house placements, dignity, aspects, or divisional charts — only Lagna, Moon Nakshatra, and current dasha. Do not fabricate specific house-lord relationships, yogas, or divisional-chart placements you cannot see. For domain-specific yes/no questions, answer only from what the dasha planet and Lagna genuinely indicate, and say plainly that a complete reading would require the full chart data if the seeker is asking something that depends on placements you don't have.`}
