...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[MEDITATION-CHECKOUT] ${step}${detailsStr}`);
};

// Price IDs for custom channeled meditations
const PRICE_IDS = {
  single: "price_1SZovaAPsnbrivP0MH8R7lXK", // €70 - Single meditation
  double: "price_1SZowHAPsnbrivP0KiHrgbrH", // €97 - Double pack
};
...
serve(async (req) => {
  ...
  try {
    ...
    const { packageType, notes } = await req.json();
    logStep("Request data", { packageType, notes });

    if (!packageType || !PRICE_IDS[packageType as keyof typeof PRICE_IDS]) {
      throw new Error("Invalid package type. Must be 'single' or 'double'");
    }

    const priceId = PRICE_IDS[packageType as keyof typeof PRICE_IDS];
    const amountPaid = packageType === "single" ? 70 : 97;
