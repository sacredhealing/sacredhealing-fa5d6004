...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "create") {
      ...
      const enableRecording = ["off", "local", "cloud"].includes(recordingMode) ? recordingMode : "off";
      ...
      if (!roomRes.ok && enableRecording === "cloud") {
        roomRes = await fetch("https://api.daily.co/v1/rooms", {
          ...
          body: JSON.stringify({
            ...basePayload,
            properties: { ...basePayload.properties, enable_recording: "off" },
          }),
        });
      }
      ...
      if (!roomRes.ok) {
        return respond({
          ...
          hint: "Check the Daily API key and recording mode configuration.",
        });
      }
