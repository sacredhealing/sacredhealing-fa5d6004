// Ramayana Codex — extracted verbatim from RamayanaCodex.tsx
// 7 Kandas, 33 secrets. Original interpretive/esoteric commentary on
// the Ramayana, not reproduction of the source epic's text.

export type RamayanaTier = 'free' | 'prana-flow' | 'siddha-quantum' | 'akasha-infinity';
export interface RamayanaSecret {
  title: string; teaching: string; siddhaRevelation: string;
  activation: string; mantra: string; mantraTranslation?: string;
}
export interface RamayanaKanda {
  id: string; number: number; name: string; sanskrit: string; chakra: string;
  element: string; teaching: string; voice: string; tier: RamayanaTier;
  color: string; intro: string; secrets: RamayanaSecret[];
}

export const RAMAYANA_KANDAS: RamayanaKanda[] =
[
  {
  ...
  },
  {
    ...
    voice: "Rāma + Vālmīki", color: "#D4AF37",
