...
import { newSupabase } from "@/integrations/supabase/newClient";
...
interface Whale {
  ...
  win_rate_30d: number; roi_30d: number;
...
export default function PolymarketOracle() {
  ...
  async function fetchData() {
    try {
      const [t, w, s] = await Promise.all([
        newSupabase.from("polymarket_trades").select("*").order("created_at", { ascending: false }).limit(200),
        newSupabase.from("polymarket_whales").select("*").order("roi_30d", { ascending: false }),
        newSupabase.from("polymarket_bot_settings").select("*").limit(1).single(),
      ]);
      if (t.data) setTrades(t.data as Trade[]);
      if (w.data) setWhales(w.data as Whale[]);
  ...
  return (
    <div style={{minHeight:"100vh",background:"#050505",color:"#fff",fontFamily:"'Plus Jakarta Sans',sans-serif",padding:"28px 20px",maxWidth:1100,margin:"0 auto"}}>
      ...
      {loading&&<div style={{color:G,padding:40,textAlign:"center" as const}}>⟳ Fetching oracle data from new Supabase...</div>}
        ...
        <div>
          ...
          <div style={{background:`${G}09`,border:`1px solid ${G}25`,borderRadius:18,padding:20,marginBottom:18}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))",gap:14}}>
              {[["🔍 Scan","450 Polygon blocks every 15 min"],["🦈 Detect","All trades >$100 on Polymarket V2"],["📊 Filter","Entry zone 6–94% only"],["⚡ Mirror","Top 3 whale trades at 5% balance"],["📈 Compound","Position sizes grow with balance"]].map(([t,d])=>(
      ...
      {!loading&&tab==="whales"&&(
        <div>
          ...
          <div style={{overflowX:"auto" as const}}>
            <table style={{width:"100%",borderCollapse:"collapse" as const,fontSize:12}}>
              <thead><tr style={{borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
                {["Alias","Wallet","Win Rate","ROI 30d","Trades Seen","Last Active","Status"].map(h=>(
                  <th key={h} style={{padding:"8px 10px",textAlign:"left" as const,color:"rgba(255,255,255,0.3)",fontSize:9}}>{h}</th>
                ))}
              </tr></thead>
              <tbody>
                {whales.map(w=>(
                  <tr key={w.id} style={{borderBottom:"1px solid rgba(255,255,255,0.03)"}}>
                    ...
                    <td style={{padding:"8px 10px",color:"rgba(255,255,255,0.5)"}}>{w.total_trades_seen||0}</td>
      ...
      {!loading&&tab==="setup"&&(
        <div style={{display:"grid",gap:16}}>
          <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:18,padding:24}}>
            <div style={{fontSize:9,letterSpacing:"0.35em",color:"#A78BFA",marginBottom:14}}>MIGRATION PLAN — OLD → NEW SUPABASE</div>
            <div style={{display:"grid",gap:12}}>
              {[
                ["✅ Done","New Supabase project created (fjdzhrdpioxdeyyfogep)","#10B981"],
                ["✅ Done","Schema migrated — all 51 tables structure copied","#10B981"],
                ["✅ Done","Polymarket Oracle tables live on new project","#10B981"],
                ["✅ Done","Storage migrated to Cloudflare R2","#10B981"],
                ["⏳ Pending","Data migration — copy all rows from old to new Supabase","#F59E0B"],
                ["⏳ Pending","Update Vercel env vars: VITE_SUPABASE_URL + VITE_SUPABASE_PUBLISHABLE_KEY","#F59E0B"],
                ["⏳ Pending","Cancel Lovable subscription (saves €25/mo)","#F59E0B"],
              ].map(([status,desc,col])=>(
                <div key={desc} style={{display:"flex",gap:14,padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                  <span style={{color:col as string,fontWeight:700,fontSize:12,minWidth:100,flexShrink:0}}>{status}</span>
          ...
          <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:18,padding:20}}>
            ...
            {[["1. Fresh MetaMask wallet","Create a NEW wallet only for this bot. Never use your main wallet."],["2. Add Polygon (Chain 137)","RPC: polygon-mainnet.g.alchemy.com/v2/az4D7Awbl2E2rNpe6kc3M"],["3. Fund USDC on Polygon","Binance/Coinbase → withdraw to Polygon. Add ~€12 to cover gas."],["4. Polymarket account","polymarket.com → connect MetaMask → Deposit → V2 wallet auto-created"],["5. USDC → pUSD","Auto-wrapped on deposit. 1:1 ratio, no fee."],["6. Private key to GitHub","Settings → Secrets → POLYMARKET_PRIVATE_KEY → bot trades live"]].map(([s,d])=>(
