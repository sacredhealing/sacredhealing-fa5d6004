...
const MeditationPlaylists: React.FC = () => {
  ...
  const load = async () => {
    ...
    for (const pl of pls || []) {
      ...
      cards.push({ id: pl.id, name: pl.name, itemCount: count || 0 });
    }
    setPlaylists(cards);
    setLoading(false);
  };
  ...
  const createPlaylist = async () => {
    ...
    setNewName('');
    navigate(`/meditation-playlists/${data.id}`);
  };

  return (
    <div style={{ background: '#050505', minHeight: '100vh', paddingBottom: 100, color: '#fff', fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
      <div style={{ padding: '20px 20px 8px' }}>
        <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: '-0.02em' }}>Your Meditation Playlists</div>
      </div>
      ...
      {!loading && playlists.length === 0 && (
        <div style={{ textAlign: 'center', padding: '50px 30px', color: 'rgba(255,255,255,.4)', fontSize: 13, lineHeight: 1.6 }}>
          No playlists yet. Create one above, or tap "Add to Playlist" on any meditation in Dhyana.
        </div>
      )}

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, padding: '0 20px' }}>
        {playlists.map((pl) => (
          <div key={pl.id} onClick={() => navigate(`/meditation-playlists/${pl.id}`)} style={{ cursor: 'pointer' }}>
            <div style={{
              ...
              <Sparkles size={28} style={{ color: 'rgba(212,175,55,.5)' }} />
            </div>
            ...
            <div style={{ fontSize: 11.5, color: 'rgba(255,255,255,.4)', marginTop: 1 }}>
              {pl.itemCount} {pl.itemCount === 1 ? 'meditation' : 'meditations'}
            </div>
          </div>
        ))}
      </div>
