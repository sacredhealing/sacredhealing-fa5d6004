...
import { useHolyScienceProgress } from '@/hooks/useHolyScienceProgress';
...
import HolyScienceModuleContent from '@/components/HolyScienceModuleContent';

export default function HolyScienceModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useHolyScienceProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/holy-science/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Holy Science Curriculum"
      academyHref="/holy-science"
      courseTitle="Kaivalya Darsanam Decoded"
      courseIcon={<BookOpen size={24} />}
      ...
      prevHref={prevModule ? `/holy-science/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/holy-science/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <HolyScienceModuleContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
