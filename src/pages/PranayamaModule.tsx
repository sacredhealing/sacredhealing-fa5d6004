...
import { usePranayamaProgress } from '@/hooks/usePranayamaProgress';
...
import { cyan, fade } from '@/components/education/tokens';
import PranayamaModuleContent from '@/components/PranayamaModuleContent';

export default function PranayamaModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = usePranayamaProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/thirumoolar-pranayama/module/${c.id}` };
        }),
      };
    });
  ...
  const accent = cyan(0.9);
  const salesHref = getSalesPageForRank(getCourseTierRequiredRank(module.tier_required));

  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Thirumoolar's Pranayama Codex"
      academyHref="/thirumoolar-pranayama"
      courseTitle="Pranayama Codex"
      ...
      prevHref={prevModule ? `/thirumoolar-pranayama/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/thirumoolar-pranayama/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <PranayamaModuleContent moduleId={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
