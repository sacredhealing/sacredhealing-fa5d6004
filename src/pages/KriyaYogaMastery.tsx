...
export default function KriyaYogaMastery() {
  ...
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = useKriyaYogaProgress(membershipReady);
    ...
    return TIER_ORDER.map((t) => {
      ...
      return {
        ...
        lessons: mods.map((m) => {
          ...
          const allowed = hasFeatureAccess(isAdmin, tier, getCourseTierRequiredRank(m.tier_required), grantedAcademies.has('kriya_yoga_academy'));
    ...
    <div style={{ minHeight: '100vh', background: '#050505', color: 'rgba(255,255,255,0.9)', paddingBottom: 104 }}>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        <CourseSyllabus
          ...
          courseTitle="Kriya Yoga Mastery"
          academyName="Kriya Yoga Mastery"
          progressLabel={`${stats.completedModules} / ${courses.length || 11} · ${stats.completionPercent}%`}
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/kriya-yoga/module/${lessonId}`);
            }
          }}
        />
