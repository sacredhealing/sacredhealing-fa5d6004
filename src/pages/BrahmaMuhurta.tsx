...
const violet = (a: number) => `rgba(232,121,249,${a})`;

// ─── TIER HELPERS ─────────────────────────────────────────────────────────────
const TIER_RANK: Record<string, number> = {
  free: 0, 'prana-flow': 1, 'siddha-quantum': 2, 'akasha-infinity': 3,
};
function tierRank(tier: string | null): number {
  if (!tier) return 0;
  const t = tier.toLowerCase();
  if (t.includes('akasha'))  return 3;
  if (t.includes('siddha'))  return 2;
  if (t.includes('prana'))   return 1;
  return 0;
}

// ─── TYPES ────────────────────────────────────────────────────────────────────
interface SecretTeaching { label: string; body: string; }
interface ModuleStep { num: number; title: string; time: string; desc: string; }
interface DataPoint { label: string; value: string; }
interface MantraItem { label: string; text: string; meaning: string; }
interface Module {
  id: number;
...
// ─── MAIN COMPONENT ───────────────────────────────────────────────────────────
export default function BrahmaMuhurta() {
  ...
  const userRank = isAdmin ? 3 : tierRank(tier);

  useEffect(() => {
    const id = setInterval(() => setGlowPhase(p => (p + 1) % 360), 60);
    return () => clearInterval(id);
  }, []);

  const glowOpacity = (0.12 + 0.05 * Math.sin((glowPhase * Math.PI) / 180)).toFixed(3);

  const tierColor = (t: string) => {
    if (t === 'akasha') return violet(0.9);
    if (t === 'siddha') return gold(0.9);
    if (t === 'prana')  return cyan(0.9);
    return white(0.5);
  };
  const tierLabel = (t: string) => {
    if (t === 'akasha') return 'AKASHA-INFINITY';
    if (t === 'siddha') return 'SIDDHA-QUANTUM';
    if (t === 'prana')  return 'PRANA-FLOW';
    return 'FREE';
  };
  const tierBorder = (t: string) => {
    if (t === 'akasha') return violet(0.25);
    if (t === 'siddha') return gold(0.25);
    if (t === 'prana')  return cyan(0.25);
    return white(0.12);
  };
  const modRank = (t: string) => ({ free: 0, prana: 1, siddha: 2, akasha: 3 }[t] ?? 0);
  const isLocked = (m: Module) => modRank(m.tier) > userRank;
  ...
  return (
    <div style={{ background: '#050505', minHeight: '100vh', paddingBottom: 100, maxWidth: 430, margin: '0 auto', position: 'relative', overflow: 'hidden' }}>

      {/* Halo */}
      <div style={{
        position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none',
        background: `radial-gradient(ellipse 80% 40% at 50% -5%, rgba(212,175,55,${glowOpacity}) 0%, transparent 65%)`,
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none' }}>
        {Array.from({ length: 30 }).map((_, i) => (
          <div key={i} style={{
            ...
            animation: `bm-pulse ${3 + (i % 3)}s ${(i % 4) * 0.5}s ease-in-out infinite alternate`,
          }} />
        ))}
      </div>

      <style>{`
        @keyframes bm-pulse { 0%{opacity:0.05} 100%{opacity:0.3} }
        @keyframes bm-fade  { from{opacity:0;transform:translateY(12px)} to{opacity:1;transform:none} }
        @keyframes bm-glow  { 0%,100%{opacity:.6} 50%{opacity:1} }
      `}</style>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '0 16px 0' }}>
        ...
        <div style={{ padding: '48px 0 0' }}>
          <button onClick={() => navigate(-1)} style={{ ...LABEL, background: 'none', border: 'none', cursor: 'pointer', color: gold(0.4), padding: 0, marginBottom: 24 }}>
            ← Back
          </button>
        </div>

        {/* Hero */}
        <div style={{ textAlign: 'center', paddingBottom: 32, animation: 'bm-fade 0.45s ease both' }}>
          <div style={{ ...LABEL, marginBottom: 16, fontSize: 8 }}>Akashic Transmission · Siddha Quantum Nexus · 2050</div>
          <h1 style={{
            fontFamily: font, fontWeight: 900, letterSpacing: '-0.04em',
            fontSize: '2.8rem', lineHeight: 0.95, margin: '0 0 8px',
            background: 'linear-gradient(135deg,#D4AF37 0%,#FFF8C8 38%,#D4AF37 65%,#A07A10 100%)',
            WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
          }}>
            BRAHMA<br/>MUHURTA
          </h1>
          <div style={{ fontFamily: font, fontSize: 10, fontWeight: 800, letterSpacing: '0.2em', color: white(0.35), textTransform: 'uppercase', marginBottom: 16 }}>
            The Hour of the Creator
          </div>
          <p style={{ fontFamily: serif, fontStyle: 'italic', fontSize: '0.92rem', color: white(0.4), lineHeight: 1.7, marginBottom: 20 }}>
            The complete Siddha transmission on the sacred pre-dawn window — cosmology, neuroscience, Nadi science, secret mantras, Kala Vortex mechanics and direct initiations from Agastyar, Thirumoolar, Bogar & Mahavatar Babaji.
          </p>
          <div style={{
            ...
            <span>☀</span>
            <span style={{ ...LABEL, fontSize: 8 }}>96–48 Min Before Local Sunrise</span>
          </div>
        </div>

        {/* Divider */}
        <div style={{ height: 1, background: 'linear-gradient(90deg,transparent,rgba(212,175,55,0.35),transparent)', marginBottom: 28 }} />

        {/* Modules */}
        {(['free','prana','siddha','akasha'] as const).map(section => {
          const sectionMods = MODULES.filter(m => m.tier === section);
          const sectionColor = tierColor(section);
          const sectionLabel = tierLabel(section);
          return (
            <div key={section} style={{ marginBottom: 8 }}>
              {/* Section heading */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '24px 0 14px' }}>
                <span style={{
                  fontFamily: font, fontSize: 8, fontWeight: 800, letterSpacing: '0.4em',
                  textTransform: 'uppercase', color: sectionColor,
                  background: `${sectionColor.replace('0.9)', '0.1)')}`,
                  border: `1px solid ${sectionColor.replace('0.9)', '0.25)')}`,
                  padding: '4px 14px', borderRadius: 100,
                }}>
                  {sectionLabel}
                </span>
                <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.05)' }} />
                <span style={{ ...LABEL, fontSize: 8, color: white(0.2) }}>{sectionMods.length} MODULES</span>
              </div>

              {sectionMods.map((mod, idx) => {
                const locked = isLocked(mod);
                const open = expandedId === mod.id;
                const secretShown = revealedSecrets[mod.id];
                const mColor = tierColor(mod.tier);

                return (
                  <div key={mod.id} style={{
                    ...GLASS,
                    marginBottom: 12,
                    border: open ? `1px solid ${tierBorder(mod.tier)}` : `1px solid rgba(255,255,255,0.06)`,
                    boxShadow: open ? `0 0 40px ${mColor.replace('0.9)', '0.1)')}` : 'none',
                    opacity: locked ? 0.45 : 1,
                    transition: 'all 0.35s',
                    animation: `bm-fade 0.45s ${idx * 0.08}s ease both`,
                  }}>
                    {/* Header */}
                    <div
                      onClick={() => !locked && setExpandedId(open ? null : mod.id)}
                      style={{ padding: '22px 20px', cursor: locked ? 'default' : 'pointer', display: 'flex', alignItems: 'center', gap: 16 }}
                    >
                      <div style={{
                        width: 48, height: 48, borderRadius: 16, flexShrink: 0,
                        background: `${mColor.replace('0.9)', '0.1)')}`,
                        border: `1px solid ${mColor.replace('0.9)', '0.2)')}`,
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                          <span style={{ ...LABEL, fontSize: 8, color: mColor }}>{sectionLabel}</span>
                          {mod.isInitiation && (
                            <span style={{ ...LABEL, fontSize: 7, color: violet(0.85), background: violet(0.1), border: `1px solid ${violet(0.2)}`, padding: '2px 8px', borderRadius: 100 }}>INITIATION</span>
                          )}
                        </div>
                        <div style={{ fontFamily: font, fontSize: 15, fontWeight: 800, letterSpacing: '-0.02em', color: locked ? white(0.3) : white(0.9), lineHeight: 1.2 }}>
                          {mod.title}
                        </div>
                        <div style={{ fontFamily: serif, fontStyle: 'italic', fontSize: '0.82rem', color: white(0.35), marginTop: 3 }}>
                          {mod.subtitle}
                        </div>
                      </div>
                      {!locked && (
                        <div style={{ fontSize: 14, color: mColor, opacity: 0.5, transition: 'transform 0.3s', transform: open ? 'rotate(180deg)' : 'none' }}>↓</div>
                      )}
                      {locked && (
                        <div style={{ ...LABEL, fontSize: 7, color: white(0.25), border: `1px solid ${white(0.1)}`, padding: '3px 10px', borderRadius: 100 }}>
                          {sectionLabel.split('-')[0]} TIER
                        </div>
                      )}
                    </div>
                    ...
                    {open && !locked && (
                      <div style={{ padding: '0 20px 24px' }}>
                        <div style={{ height: 1, background: `linear-gradient(90deg,${mColor.replace('0.9)','0.35)')},transparent)`, marginBottom: 20 }} />

                        {/* Body text */}
                        <p style={{ fontFamily: serif, fontStyle: 'italic', fontSize: '0.93rem', lineHeight: 1.9, color: white(0.68), whiteSpace: 'pre-line', marginBottom: 0 }}>
                          {mod.body}
                        </p>
                        ...
                        {mod.data && (
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 20 }}>
                            {mod.data.map(d => (
                              <div key={d.label} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '14px 16px' }}>
                                <div style={{ ...LABEL, fontSize: 8, color: white(0.3), marginBottom: 5 }}>{d.label}</div>
                                <div style={{ fontFamily: font, fontSize: 14, fontWeight: 800, color: gold(0.9) }}>{d.value}</div>
                              </div>
                            ))}
                          </div>
                        )}
                        ...
                        {/* Inner section */}
                        {mod.innerTitle && mod.innerBody && (
                          <div style={{ marginTop: 20, padding: '18px 20px', background: 'rgba(255,255,255,0.015)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 18 }}>
                            <div style={{ ...LABEL, color: mColor, marginBottom: 12 }}>{mod.innerTitle}</div>
                            <p style={{ fontFamily: serif, fontStyle: 'italic', fontSize: '0.88rem', lineHeight: 1.85, color: white(0.65), whiteSpace: 'pre-line' }}>{mod.innerBody}</p>
                          </div>
                        )}
                        ...
                        {mod.steps && (
                          <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
                            {mod.steps.map(step => (
                              <div key={step.num} style={{ display: 'flex', gap: 14, padding: '16px 18px', background: 'rgba(255,255,255,0.018)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16 }}>
                                <div style={{ width: 34, height: 34, borderRadius: 11, background: 'rgba(212,175,55,0.12)', border: '1px solid rgba(212,175,55,0.22)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: font, fontSize: 12, fontWeight: 900, color: gold(0.9), flexShrink: 0 }}>
                                  {step.num}
                                </div>
                                <div>
                                  <div style={{ fontFamily: font, fontSize: 13, fontWeight: 800, color: white(0.9), marginBottom: 3 }}>{step.title}</div>
                                  <div style={{ ...LABEL, fontSize: 8, color: gold(0.5), marginBottom: 6 }}>{step.time}</div>
                                  <p style={{ fontFamily: serif, fontStyle: 'italic', fontSize: '0.85rem', lineHeight: 1.75, color: white(0.6) }}>{step.desc}</p>
                                </div>
                              </div>
                            ))}
                          </div>
                        ...
                        {mod.mantras && (
                          <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
                            {mod.mantras.map((m, i) => (
                              <div key={i} style={{
                                padding: '22px 20px',
                                background: `linear-gradient(135deg,${mColor.replace('0.9)','0.07)')},rgba(212,175,55,0.03))`,
                                border: `1px solid ${mColor.replace('0.9)','0.2)')}`,
                                borderRadius: 20, textAlign: 'center',
                              }}>
                                <div style={{ ...LABEL, color: mColor, marginBottom: 14 }}>{m.label}</div>
                                <div style={{ fontFamily: font, fontWeight: 700, fontSize: '1rem', color: '#FFF8D6', letterSpacing: '0.06em', lineHeight: 1.5, marginBottom: 14 }}>{m.text}</div>
                                <p style={{ fontFamily: serif, fontStyle: 'italic', fontSize: '0.85rem', color: white(0.5), lineHeight: 1.7, whiteSpace: 'pre-line', textAlign: 'left' }}>{m.meaning}</p>
                              </div>
                            ))}
                          </div>
                        )}
                        ...
                        {/* Secret reveal */}
                        {mod.secret && !secretShown && (
                          <button
                            onClick={() => setRevealedSecrets(prev => ({ ...prev, [mod.id]: true }))}
                            style={{
                              width: '100%', marginTop: 20, padding: '16px',
                              borderRadius: 16, border: `1px dashed ${mColor.replace('0.9)','0.35)')}`,
                              background: mColor.replace('0.9)', '0.04)'),
                              color: mColor, cursor: 'pointer',
                              fontFamily: font, fontSize: 9, fontWeight: 800, letterSpacing: '0.4em',
                              textTransform: 'uppercase' as const,
                            }}
                          >
                            {mod.secret.label}
                        ...
                        {mod.secret && secretShown && (
                          <div style={{
                            marginTop: 20, padding: '20px 22px', borderRadius: 18,
                            background: `linear-gradient(135deg,${mColor.replace('0.9)','0.07)')},${mColor.replace('0.9)','0.02)')})`,
                            border: `1px solid ${mColor.replace('0.9)','0.22)')}`,
                          }}>
                            <div style={{ ...LABEL, color: mColor, marginBottom: 12 }}>{mod.secret.label}</div>
                            <p style={{ fontFamily: serif, fontStyle: 'italic', fontSize: '0.88rem', lineHeight: 1.85, color: white(0.75), whiteSpace: 'pre-line' }}>{mod.secret.body}</p>
                          </div>
                        )}
                      </div>
                    )}
        ...
        {/* Footer */}
        <div style={{ height: 1, background: 'linear-gradient(90deg,transparent,rgba(212,175,55,0.25),transparent)', margin: '32px 0 28px' }} />
        <div style={{ textAlign: 'center', paddingBottom: 20 }}>
          <div style={{ fontSize: 28, marginBottom: 14, animation: 'bm-glow 4s ease-in-out infinite' }}>🕉</div>
          <p style={{ fontFamily: serif, fontStyle: 'italic', fontSize: '0.88rem', color: white(0.28), lineHeight: 1.9 }}>
            This transmission is sealed with the Shakti-current of the 18 Siddhas.<br/>
            Reading it is itself an initiation. The Akasha has received your attention.<br/>
            Your Sadhana begins at the next Brahma Muhurta.
          </p>
          <div style={{ ...LABEL, marginTop: 20, fontSize: 8, color: gold(0.25) }}>
            Siddha Quantum Nexus · Shiva Siddhananda · 2050
          </div>
        </div>
      </div>
    </div>
