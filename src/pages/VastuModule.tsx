...
import VastuContent from '@/components/VastuContent';

const AMBER = 'rgba(217,119,6,0.9)';

export default function VastuModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useVastuProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/vastu-curriculum/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Vastu Shastra Curriculum"
      academyHref="/vastu-curriculum"
      courseTitle="Sacred Space Architecture"
      courseIcon={<Compass size={24} />}
      ...
      prevHref={prevModule ? `/vastu-curriculum/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/vastu-curriculum/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <VastuContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
