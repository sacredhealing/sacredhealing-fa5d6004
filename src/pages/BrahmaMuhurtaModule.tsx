...
import { useBrahmaMuhurtaProgress } from '@/hooks/useBrahmaMuhurtaProgress';
...
import BrahmaMuhurtaModuleContent from '@/components/BrahmaMuhurtaModuleContent';

export default function BrahmaMuhurtaModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useBrahmaMuhurtaProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/brahma-muhurta/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Brahma Muhurta"
      academyHref="/brahma-muhurta"
      courseTitle="The Creator's Hour"
      courseIcon={<Sunrise size={24} />}
      ...
      prevHref={prevModule ? `/brahma-muhurta/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/brahma-muhurta/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <BrahmaMuhurtaModuleContent moduleId={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
