...
export default function SiddhaAbundanceCurriculum() {
  ...
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = useAbundanceSadhanaProgress(membershipReady);
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        <CourseSyllabus
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/abundance-curriculum/module/${lessonId}`);
            }
          }}
        />
        ...
        <footer style={{ marginTop: 40, paddingBottom: 32, textAlign: 'center' }}>
          <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.45em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)' }}>
            Lakshmi · Kubera · Pachamama · The 18 Siddhas
          </p>
        </footer>
      </div>
    </div>
