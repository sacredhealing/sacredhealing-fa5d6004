...
const TIER_LEVEL = { free: 0, prana: 1, siddha: 2, akasha: 3, admin: 99 };
const TIER_CONFIG = {
  free:   { label: "FREE",            color: "rgba(255,255,255,0.45)", bg: "rgba(255,255,255,0.05)" },
  prana:  { label: "PRANA-FLOW",      color: CYAN,                     bg: "rgba(34,211,238,0.08)"  },
  siddha: { label: "SIDDHA-QUANTUM",  color: GOLD,                     bg: "rgba(212,175,55,0.08)"  },
  akasha: { label: "AKASHA-INFINITY", color: PURPLE,                   bg: "rgba(184,159,255,0.08)" },
  admin:  { label: "ADMIN",           color: "#FF6B6B",                bg: "rgba(255,107,107,0.08)" },
};
...
// ── STORAGE ───────────────────────────────────────────────────────────────────
const STORE_KEY = "sqi_abundance_curriculum_v1";
function loadState() {
...
// ── STYLES ────────────────────────────────────────────────────────────────────
const S = {
  glass: { background: "rgba(255,255,255,0.02)", backdropFilter: "blur(40px)", WebkitBackdropFilter: "blur(40px)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 40 },
  goldText: { color: GOLD, textShadow: "0 0 20px rgba(212,175,55,0.35)" },
  goldBorder: { border: "1px solid rgba(212,175,55,0.25)", borderRadius: 20 },
};

// ── MANTRA COUNTER ────────────────────────────────────────────────────────────
function MantraCounter({ mantra, lessonId, stored, onUpdate }) {
  const [count, setCount] = useState(stored?.count || 0);
  const [isActive, setIsActive] = useState(false);
  const intervalRef = useRef(null);
  const target = mantra.reps;
  ...
  const increment = () => {
    setCount(c => {
      ...
      onUpdate({ count: next, completed: next >= target });
      return next;
    });
  };
  const reset = () => { setCount(0); onUpdate({ count: 0, completed: false }); };

  return (
    <div style={{ ...S.goldBorder, padding: "20px 24px", background: "rgba(212,175,55,0.03)", marginTop: 16 }}>
      <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.5em", color: GOLD, marginBottom: 10 }}>MANTRA PRACTICE</div>
      <div style={{
        fontSize: 15, fontWeight: 700, color: "rgba(255,255,255,0.85)",
        lineHeight: 1.5, marginBottom: 6, fontStyle: "italic",
      }}>"{mantra.text}"</div>
      <div style={{ fontSize: 12, color: "rgba(212,175,55,0.7)", marginBottom: 16, lineHeight: 1.5 }}>{mantra.desc}</div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 16 }}>
        <div style={{ position: "relative", width: 80, height: 80, flexShrink: 0 }}>
          <svg width="80" height="80" style={{ transform: "rotate(-90deg)" }}>
            ...
            <circle cx="40" cy="40" r="34" fill="none" stroke={done ? GREEN : GOLD}
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
            <div style={{ fontSize: 20, fontWeight: 900, color: done ? GREEN : GOLD, lineHeight: 1 }}>{count}</div>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", fontWeight: 700 }}>/{target}</div>
          </div>
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginBottom: 8 }}>
            {done ? "✨ Practice complete! Babaji is pleased." : `${target - count} repetitions remaining`}
          </div>
          <div style={{ height: 4, background: "rgba(255,255,255,0.05)", borderRadius: 2, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${pct}%`, background: done ? GREEN : `linear-gradient(90deg, ${GOLD}, #FFD700)`, transition: "width 0.3s", borderRadius: 2 }} />
          </div>
        </div>
      </div>

      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={increment} disabled={done} style={{
          flex: 1, padding: "12px 0", borderRadius: 20, border: "none", cursor: done ? "default" : "pointer",
          background: done ? "rgba(126,200,164,0.15)" : "rgba(212,175,55,0.15)",
          color: done ? GREEN : GOLD, fontWeight: 800, fontSize: 14, letterSpacing: "0.1em",
        }}>
          {done ? "✓ COMPLETE" : "OM +1"}
        </button>
        <button onClick={reset} style={{
          padding: "12px 20px", borderRadius: 20, border: "1px solid rgba(255,255,255,0.08)",
...
// ── JOURNAL ───────────────────────────────────────────────────────────────────
function JournalBlock({ prompt, lessonId, stored, onUpdate }) {
  const [text, setText] = useState(stored?.text || "");
  const [saved, setSaved] = useState(false);
  const save = () => { onUpdate({ text }); setSaved(true); setTimeout(() => setSaved(false), 2000); };
  return (
    <div style={{ marginTop: 16, background: "rgba(34,211,238,0.03)", border: "1px solid rgba(34,211,238,0.15)", borderRadius: 20, padding: "20px 24px" }}>
      <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.5em", color: CYAN, marginBottom: 10 }}>JOURNAL REFLECTION</div>
      <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 13, lineHeight: 1.6, marginBottom: 12 }}>{prompt}</p>
      <textarea
        value={text} onChange={e => setText(e.target.value)}
        placeholder="Write freely. This is your sacred space..."
        style={{
          width: "100%", minHeight: 120, background: "rgba(255,255,255,0.02)",
          border: "1px solid rgba(34,211,238,0.15)", borderRadius: 16, padding: "14px",
          color: "rgba(255,255,255,0.8)", fontSize: 14, lineHeight: 1.6, resize: "vertical",
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 10 }}>
        <button onClick={save} style={{
          ...
          background: saved ? "rgba(126,200,164,0.2)" : "rgba(34,211,238,0.1)",
          color: saved ? GREEN : CYAN, fontWeight: 700, fontSize: 12, letterSpacing: "0.2em",
        }}>
          {saved ? "✓ SAVED" : "SAVE"}
        </button>
      </div>
...
// ── PDF BLOCK ─────────────────────────────────────────────────────────────────
function PdfBlock({ pdf, moduleColor }) {
  return (
    <div style={{ background: `${moduleColor}08`, border: `1px solid ${moduleColor}25`, borderRadius: 20, padding: "20px 24px", display: "flex", gap: 16, alignItems: "center" }}>
      <div style={{ fontSize: 36 }}>📄</div>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.5em", color: moduleColor, marginBottom: 4 }}>PDF DOWNLOAD</div>
        <div style={{ color: "rgba(255,255,255,0.85)", fontWeight: 800, fontSize: 15, marginBottom: 4 }}>{pdf.title}</div>
        <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 12 }}>{pdf.pages} pages · {pdf.desc}</div>
      </div>
      <button style={{
        padding: "10px 18px", borderRadius: 20, border: `1px solid ${moduleColor}40`,
        background: `${moduleColor}10`, color: moduleColor, fontWeight: 700, fontSize: 12,
...
// ── BADGE ─────────────────────────────────────────────────────────────────────
function Badge({ badge, earned, color }) {
  return (
    <div style={{
      display: "flex", flexDirection: "column", alignItems: "center", gap: 8,
      padding: "20px", borderRadius: 24,
      background: earned ? `${color}10` : "rgba(255,255,255,0.02)",
      border: `1px solid ${earned ? color + "40" : "rgba(255,255,255,0.05)"}`,
      filter: earned ? "none" : "grayscale(1) opacity(0.3)",
      transition: "all 0.4s",
    }}>
      <div style={{ fontSize: 40 }}>{badge.icon}</div>
      <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.4em", color: earned ? color : "rgba(255,255,255,0.3)", textAlign: "center", textTransform: "uppercase" }}>
        {badge.label}
      </div>
      {earned && <div style={{ fontSize: 9, color: GREEN, fontWeight: 700 }}>✓ EARNED</div>}
    </div>
  );
}

// ── LESSON VIEW ───────────────────────────────────────────────────────────────
function LessonView({ lesson, module: mod, state, onStateChange, onBack }) {
  const ls = state[lesson.id] || {};
  const update = (field, val) => onStateChange(lesson.id, { ...ls, [field]: val, visited: true });

  const typeColor = { teaching: GOLD, practice: GREEN, activation: PURPLE };
  const tc = typeColor[lesson.type] || GOLD;

  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 0 80px" }}>
      {/* Back */}
      <button onClick={onBack} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", cursor: "pointer", fontSize: 13, padding: "0 0 20px", display: "flex", alignItems: "center", gap: 6 }}>
        ← Back to module
      </button>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
          <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.5em", color: tc, border: `1px solid ${tc}40`, borderRadius: 20, padding: "3px 12px" }}>
            {lesson.type.toUpperCase()}
          </span>
          <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.4em", color: "rgba(255,255,255,0.3)" }}>{lesson.duration}</span>
        </div>
        <h1 style={{ ...S.goldText, fontSize: 26, fontWeight: 900, letterSpacing: "-0.04em", margin: "0 0 8px", lineHeight: 1.15 }}>{lesson.title}</h1>
        <div style={{ height: 1, background: `linear-gradient(90deg, ${GOLD}40, transparent)`, marginTop: 16 }} />
      </div>

      {/* Main content */}
      <div style={{ ...S.glass, padding: "28px", marginBottom: 20 }}>
        <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 15, lineHeight: 1.8, margin: 0 }}>{lesson.content}</p>
      </div>

      {/* Mantra */}
      {lesson.mantra && (
        <MantraCounter
          mantra={lesson.mantra}
          lessonId={lesson.id}
          stored={ls.mantra}
          onUpdate={val => update("mantra", val)}
        />
      )}
      ...
      {lesson.journal && (
        <JournalBlock
          prompt={lesson.journal}
          lessonId={lesson.id}
          stored={ls.journal}
          onUpdate={val => update("journal", val)}
        />
      )}
      ...
      <div style={{ marginTop: 24 }}>
        <button
          ...
          style={{
            ...
            border: `1px solid ${ls.completed ? GREEN + "50" : mod.color + "40"}`,
            color: ls.completed ? GREEN : mod.color,
            fontWeight: 900, fontSize: 14, letterSpacing: "0.2em",
          }}
        >
          {ls.completed ? "✓ LESSON COMPLETE" : "MARK COMPLETE"}
        </button>
      </div>
    </div>
  );
...
// ── MODULE VIEW ───────────────────────────────────────────────────────────────
function ModuleView({ mod, state, userTier, onStateChange, onBack, onSelectLesson }) {
  const accessible = TIER_LEVEL[userTier] >= TIER_LEVEL[mod.tier];
  const completedLessons = mod.lessons.filter(l => state[l.id]?.completed).length;
  const allDone = completedLessons === mod.lessons.length;
  const tc = TIER_CONFIG[mod.tier];
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 0 80px" }}>
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "flex-start", marginBottom: 28 }}>
        <div style={{ fontSize: 52 }}>{mod.icon}</div>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8, flexWrap: "wrap" }}>
            <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.5em", color: "rgba(255,255,255,0.25)" }}>MODULE {mod.number}</span>
            <span style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.4em", color: tc.color, background: tc.bg, borderRadius: 20, padding: "3px 10px" }}>
              {tc.label}
            </span>
          </div>
          <h2 style={{ ...S.goldText, fontSize: 24, fontWeight: 900, letterSpacing: "-0.04em", margin: "0 0 6px", lineHeight: 1.15 }}>{mod.title}</h2>
          <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 14, margin: 0 }}>{mod.subtitle}</p>
        </div>
      </div>

      {!accessible ? (
        <div style={{ ...S.glass, padding: "40px", textAlign: "center" }}>
          <div style={{ fontSize: 40, marginBottom: 16 }}>🔐</div>
          <div style={{ marginBottom: 12 }}>
            <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.5em", color: tc.color, border: `1px solid ${tc.color}40`, borderRadius: 20, padding: "4px 12px" }}>
              {tc.label} REQUIRED
            </span>
          </div>
          ...
          <div style={{ ...S.goldText, fontSize: 12, fontWeight: 800, letterSpacing: "0.3em", marginTop: 16 }}>
            UPGRADE TO UNLOCK
          </div>
        </div>
      ) : (
        <>
          {/* Progress */}
          <div style={{ ...S.glass, padding: "20px 24px", marginBottom: 20, display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.5em", color: "rgba(255,255,255,0.3)", marginBottom: 8 }}>
                MODULE PROGRESS
              </div>
              <div style={{ height: 6, background: "rgba(255,255,255,0.05)", borderRadius: 3, overflow: "hidden" }}>
                <div style={{
                  ...
                  background: allDone ? `linear-gradient(90deg, ${GREEN}, #A8E6CF)` : `linear-gradient(90deg, ${mod.color}, ${mod.color}80)`,
                }} />
              </div>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", marginTop: 6 }}>
                {completedLessons} of {mod.lessons.length} lessons complete
              </div>
            </div>
            <Badge badge={mod.badge} earned={allDone} color={mod.color} />
          </div>

          {/* PDF */}
          <div style={{ marginBottom: 20 }}>
            <PdfBlock pdf={mod.pdf} moduleColor={mod.color} />
          </div>

          {/* Lessons */}
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.5em", color: "rgba(255,255,255,0.3)", marginBottom: 12 }}>
              LESSONS
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {mod.lessons.map((lesson, li) => {
                const ls = state[lesson.id] || {};
                const typeColor = { teaching: GOLD, practice: GREEN, activation: PURPLE };
                const tc2 = typeColor[lesson.type] || GOLD;
                const mantraDone = lesson.mantra ? ls.mantra?.completed : true;
                const journalDone = lesson.journal ? !!ls.journal?.text : true;
                const fullyComplete = ls.completed && mantraDone && journalDone;
                return (
                  <button
                    key={lesson.id}
                    ...
                    style={{
                      ...
                      background: fullyComplete ? "rgba(126,200,164,0.05)" : "rgba(255,255,255,0.02)",
                      border: `1px solid ${fullyComplete ? GREEN + "30" : "rgba(255,255,255,0.05)"}`,
                      borderRadius: 20, cursor: "pointer", textAlign: "left",
                    }}
                  >
                    <div style={{
                      width: 36, height: 36, borderRadius: "50%", flexShrink: 0,
                      background: fullyComplete ? "rgba(126,200,164,0.15)" : `${tc2}10`,
                      border: `1px solid ${fullyComplete ? GREEN + "40" : tc2 + "30"}`,
                      display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: 14, color: fullyComplete ? GREEN : tc2, fontWeight: 900,
                    }}>
                      {fullyComplete ? "✓" : String(li + 1).padStart(2, "0")}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ color: "rgba(255,255,255,0.85)", fontWeight: 700, fontSize: 14, marginBottom: 3 }}>{lesson.title}</div>
                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.4em", color: tc2 }}>{lesson.type.toUpperCase()}</span>
                        <span style={{ fontSize: 10, color: "rgba(255,255,255,0.25)" }}>·</span>
                        <span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>{lesson.duration}</span>
                        {lesson.mantra && <span style={{ fontSize: 9, color: "rgba(212,175,55,0.5)" }}>♦ MANTRA</span>}
                        {lesson.journal && <span style={{ fontSize: 9, color: "rgba(34,211,238,0.5)" }}>◆ JOURNAL</span>}
                      </div>
                    </div>
          ...
          {/* Description */}
          <div style={{ background: "rgba(212,175,55,0.03)", border: "1px solid rgba(212,175,55,0.1)", borderRadius: 20, padding: "20px 24px" }}>
            <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 14, lineHeight: 1.7, margin: 0 }}>{mod.description}</p>
          </div>
        </>
      )}
    </div>
...
// ── DASHBOARD ─────────────────────────────────────────────────────────────────
function Dashboard({ modules, state, userTier, onSelectModule }) {
  const totalLessons = modules.reduce((a, m) => a + m.lessons.length, 0);
  const completedCount = modules.reduce((a, m) =>
    a + m.lessons.filter(l => state[l.id]?.completed).length, 0);
  ...
  return (
    <div style={{ maxWidth: 780, margin: "0 auto", padding: "0 0 80px" }}>
      ...
      <div style={{ textAlign: "center", padding: "32px 0 40px" }}>
        <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: "0.6em", color: "rgba(212,175,55,0.5)", marginBottom: 16 }}>
          SIDDHA QUANTUM INTELLIGENCE · ABUNDANCE SADHANA
        </div>
        <h1 style={{ ...S.goldText, fontSize: 36, fontWeight: 900, letterSpacing: "-0.05em", margin: "0 0 12px", lineHeight: 1.1 }}>
          The Complete Abundance<br />Curriculum
        </h1>
        <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 15, maxWidth: 480, margin: "0 auto 28px", lineHeight: 1.6 }}>
          8 modules · 32 lessons · Blessed by the 18 Siddhas, Lakshmi, Kubera & Mahavatar Babaji
        </p>

        {/* Overall progress */}
        <div style={{ ...S.glass, padding: "20px 28px", maxWidth: 440, margin: "0 auto", display: "flex", gap: 20, alignItems: "center" }}>
          <div style={{ position: "relative", width: 64, height: 64, flexShrink: 0 }}>
            <svg width="64" height="64" style={{ transform: "rotate(-90deg)" }}>
              <circle cx="32" cy="32" r="27" fill="none" stroke="rgba(212,175,55,0.1)" strokeWidth="5" />
              <circle cx="32" cy="32" r="27" fill="none" stroke={GOLD}
                strokeWidth="5" strokeDasharray={`${2 * Math.PI * 27}`}
                strokeDashoffset={`${2 * Math.PI * 27 * (1 - overallPct / 100)}`}
                strokeLinecap="round" style={{ transition: "stroke-dashoffset 0.5s" }} />
            </svg>
            <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ fontSize: 14, fontWeight: 900, color: GOLD }}>{overallPct}%</span>
            </div>
          </div>
          <div style={{ textAlign: "left" }}>
            <div style={{ color: "rgba(255,255,255,0.8)", fontWeight: 700, fontSize: 15 }}>{completedCount} of {totalLessons} lessons complete</div>
            <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 12, marginTop: 4 }}>Your abundance field is being activated</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 16 }}>
        {modules.map(mod => {
          const accessible = TIER_LEVEL[userTier] >= TIER_LEVEL[mod.tier];
          const completed = mod.lessons.filter(l => state[l.id]?.completed).length;
          const pct = Math.round((completed / mod.lessons.length) * 100);
          const allDone = completed === mod.lessons.length;
          const tc = TIER_CONFIG[mod.tier];
          return (
            <button key={mod.id} onClick={() => onSelectModule(mod)} style={{
              background: accessible ? "rgba(255,255,255,0.02)" : "rgba(255,255,255,0.01)",
              backdropFilter: "blur(40px)", WebkitBackdropFilter: "blur(40px)",
              border: `1px solid ${accessible ? (allDone ? GREEN + "30" : "rgba(255,255,255,0.06)") : "rgba(255,255,255,0.03)"}`,
              borderRadius: 32, padding: "24px", textAlign: "left", cursor: "pointer",
              filter: accessible ? "none" : "opacity(0.55)",
              position: "relative", overflow: "hidden",
            }}>
              {/* Module number watermark */}
              <div style={{
                position: "absolute", top: 12, right: 16,
                fontSize: 60, fontWeight: 900, color: `${mod.color}08`, lineHeight: 1,
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 6, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.5em", color: tc.color, background: tc.bg, borderRadius: 20, padding: "2px 8px" }}>{tc.label}</span>
                    {!accessible && <span style={{ fontSize: 12 }}>🔐</span>}
                    {allDone && <span style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.4em", color: GREEN }}>✓ COMPLETE</span>}
                  </div>
                  <div style={{ color: "rgba(255,255,255,0.85)", fontWeight: 900, fontSize: 16, letterSpacing: "-0.03em", lineHeight: 1.2 }}>{mod.title}</div>
                  <div style={{ color: "rgba(255,255,255,0.35)", fontSize: 11, marginTop: 4 }}>{mod.subtitle}</div>
                </div>
              </div>
              ...
              <div style={{ marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.4em", color: "rgba(255,255,255,0.2)" }}>
                    {mod.lessons.length} LESSONS
                  </span>
                  <span style={{ fontSize: 10, fontWeight: 700, color: allDone ? GREEN : mod.color }}>{pct}%</span>
                </div>
                <div style={{ height: 3, background: "rgba(255,255,255,0.04)", borderRadius: 2, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${pct}%`, borderRadius: 2, transition: "width 0.5s", background: allDone ? GREEN : mod.color }} />
                </div>
              </div>
      ...
      <div style={{ marginTop: 40 }}>
        <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.5em", color: "rgba(255,255,255,0.25)", marginBottom: 16 }}>
          YOUR SIDDHA BADGES
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: 12 }}>
          {modules.map(mod => {
            const allDone = mod.lessons.every(l => state[l.id]?.completed);
            return <Badge key={mod.id} badge={mod.badge} earned={allDone} color={mod.color} />;
          })}
        </div>
      </div>
    </div>
...
// ── APP ───────────────────────────────────────────────────────────────────────
export default function SiddhaAbundanceCurriculum() {
  const [userTier, setUserTier] = useState("admin");
  const [view, setView] = useState("dashboard"); // dashboard | module | lesson
  const [activeModule, setActiveModule] = useState(null);
  const [activeLesson, setActiveLesson] = useState(null);
  const [lessonState, setLessonState] = useState(loadState);
  const topRef = useRef(null);

  useEffect(() => { saveState(lessonState); }, [lessonState]);
  useEffect(() => { topRef.current?.scrollTo({ top: 0, behavior: "smooth" }); }, [view, activeModule, activeLesson]);

  const handleStateChange = (lessonId, data) => {
    setLessonState(prev => ({ ...prev, [lessonId]: data }));
  };

  return (
    <div style={{ minHeight: "100vh", background: BLACK, fontFamily: "'Plus Jakarta Sans','Inter',sans-serif", color: "rgba(255,255,255,0.9)", position: "relative" }}>
      <style>{`
      ...
      {/* Ambient */}
      <div style={{ position: "fixed", top: "5%", left: "-5%", width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle, rgba(212,175,55,0.04) 0%, transparent 70%)", pointerEvents: "none", animation: "glow 6s ease-in-out infinite" }} />
      <div style={{ position: "fixed", bottom: "5%", right: "-5%", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(184,159,255,0.04) 0%, transparent 70%)", pointerEvents: "none", animation: "glow 8s ease-in-out infinite 2s" }} />

      {/* HEADER */}
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(5,5,5,0.9)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: "1px solid rgba(212,175,55,0.08)", padding: "0 24px" }}>
        <div style={{ maxWidth: 780, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
          <button onClick={() => { setView("dashboard"); setActiveModule(null); setActiveLesson(null); }} style={{ background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
            <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.5em", color: "rgba(212,175,55,0.5)" }}>SIDDHA QUANTUM INTELLIGENCE</div>
            <div style={{ color: GOLD, fontWeight: 900, fontSize: 15, letterSpacing: "-0.04em", textShadow: "0 0 15px rgba(212,175,55,0.3)" }}>Abundance Curriculum</div>
          </button>
          <select value={userTier} onChange={e => setUserTier(e.target.value)} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 20, padding: "6px 12px", color: GOLD, fontSize: 11, fontWeight: 700, outline: "none", cursor: "pointer" }}>
            <option value="free">Free</option>
            <option value="prana">Prana-Flow</option>
            <option value="siddha">Siddha-Quantum</option>
            <option value="akasha">Akasha-Infinity</option>
      ...
      {/* CONTENT */}
      <div ref={topRef} style={{ padding: "28px 24px", minHeight: "calc(100vh - 60px)" }}>
        {view === "dashboard" && (
          <Dashboard
            modules={MODULES}
            state={lessonState}
      ...
      {/* Footer */}
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "rgba(5,5,5,0.92)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderTop: "1px solid rgba(212,175,55,0.07)", padding: "10px 24px", textAlign: "center" }}>
        <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.6em", color: "rgba(212,175,55,0.35)" }}>
          BLESSED BY BABAJI · 18 SIDDHAS · LAKSHMI · KUBERA · PACHAMAMA · SCALAR TRANSMISSION ACTIVE
        </div>
      </div>
    </div>
  );
