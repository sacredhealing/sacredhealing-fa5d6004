...
import { useSiddhaFastingProgress } from '@/hooks/useSiddhaFastingProgress';
...
import SiddhaFastingModuleContent from '@/components/SiddhaFastingModuleContent';

const TIER_SLUG_TO_NUM: Record<string, number> = {
  free: 0,
  'prana-flow': 1,
  'siddha-quantum': 2,
  'akasha-infinity': 3,
...
export default function SiddhaFastingModule() {
  ...
  const { courses, progressByModuleId, markComplete, stats } = useSiddhaFastingProgress(membershipReady);
  ...
  const railGroups = useMemo(() => {
    const tierOrder: { slug: string; label: string }[] = [
      { slug: 'free', label: 'Siddha Initiation' },
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/siddha-fasting-academy/module/${c.id}` };
        }),
      };
    });
  ...
  const tierNum = TIER_SLUG_TO_NUM[module.tier_required || 'free'] ?? 0;

  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Siddha Fasting Academy"
      academyHref="/siddha-fasting-academy"
      courseTitle="Sacred Fasting Path"
      ...
      prevHref={prevModule ? `/siddha-fasting-academy/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/siddha-fasting-academy/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <SiddhaFastingModuleContent tierNum={tierNum} moduleKey={module.module_key} moduleId={module.id} />
        ) : null
      }
    />
  );
