...
import { useKriyaYogaProgress } from '@/hooks/useKriyaYogaProgress';
...
import KriyaYogaModuleContent from '@/components/KriyaYogaModuleContent';

export default function KriyaYogaModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useKriyaYogaProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/kriya-yoga/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Kriya Yoga Mastery"
      academyHref="/kriya-yoga"
      courseTitle="Kriya Yoga Path"
      ...
      prevHref={prevModule ? `/kriya-yoga/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/kriya-yoga/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <KriyaYogaModuleContent moduleKey={module.module_key} moduleId={module.id} />
        ) : null
      }
    />
  );
