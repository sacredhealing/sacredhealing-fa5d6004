...
export default function SiddhaSoundAlchemy() {
  ...
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = useSoundAlchemyProgress(membershipReady);
    ...
    <div style={{ minHeight: '100vh', background: '#050505', color: 'rgba(255,255,255,0.9)', paddingBottom: 104 }}>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        <CourseSyllabus
          ...
          courseTitle="Siddha Sound Alchemy"
          academyName="Siddha Sound Alchemy"
          progressLabel={`${stats.completedModules} / ${courses.length || 10} · ${stats.completionPercent}%`}
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/siddha-sound-alchemy/module/${lessonId}`);
            }
          }}
        />
        ...
        <footer style={{ marginTop: 40, paddingBottom: 32, textAlign: 'center' }}>
          <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.45em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)' }}>
            Nada Vijnana — The Siddha Science of Sound
          </p>
        </footer>
      </div>
    </div>
