...
export default function ThirumoolarPranayama() {
  ...
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = usePranayamaProgress(membershipReady);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050505', color: 'rgba(255,255,255,0.9)', paddingBottom: 104 }}>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        >
          <ArrowLeft size={14} color="rgba(34,211,238,.8)" /> Back
        </button>
        ...
        <CourseSyllabus
          ...
          courseTitle="Thirumoolar's Pranayama Codex"
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/thirumoolar-pranayama/module/${lessonId}`);
            }
          }}
        />
        ...
        <footer style={{ marginTop: 40, paddingBottom: 32, textAlign: 'center' }}>
          <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.45em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)' }}>
            3,000 Years of Tamil Siddha Breath-Science
          </p>
        </footer>
      </div>
    </div>
