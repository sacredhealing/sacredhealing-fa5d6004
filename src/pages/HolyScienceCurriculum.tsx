...
export default function HolyScienceCurriculum() {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { isAdmin } = useAdminRole();
  const { tier, loading: membershipLoading, settled } = useMembership();
  const membershipReady = !membershipLoading && settled;
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = useHolyScienceProgress(membershipReady);
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        {loadError && (
          <div style={{
            ...
            <p style={{ fontSize: 13, fontWeight: 700, color: '#F87171', margin: '0 0 4px' }}>Could not load this curriculum.</p>
        ...
        <CourseSyllabus
          ...
          courseTitle="Holy Science Curriculum"
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/holy-science/module/${lessonId}`);
            }
          }}
        />
        ...
        <footer style={{ marginTop: 40, paddingBottom: 32, textAlign: 'center' }}>
          <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.45em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)' }}>
            Kaivalya Darsanam — Sri Yukteswar's Cosmic Science
          </p>
        </footer>
      </div>
    </div>
