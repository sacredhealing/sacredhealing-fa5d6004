...
import { Lock, CheckCircle, Play, ChevronDown, ChevronUp, ArrowLeft } from 'lucide-react';
...
// TYPES
// ─────────────────────────────────────────────────────────────────────────────
type TierSlug = 'free' | 'prana-flow' | 'siddha-quantum' | 'akasha-infinity';
...
// TIER CONFIG
// ─────────────────────────────────────────────────────────────────────────────
const TIER_CONFIG: Record<TierSlug, { label: string; color: string; glow: string; rank: number; price: string; upgradeUrl: string }> = {
...
const backBtnStyle: React.CSSProperties = {
  background: 'none', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 12,
  color: 'rgba(212,175,55,0.6)', padding: '8px 16px', fontSize: 10, fontWeight: 800,
...
// SCALAR ORB
// ─────────────────────────────────────────────────────────────────────────────
const ScalarOrb: React.FC<{ size?: number }> = ({ size = 120 }) => (
  <div style={{ position: 'relative', width: size, height: size, margin: '0 auto', flexShrink: 0 }}>
    {[1,2,3,4].map(i => (
      <div key={i} style={{
        position: 'absolute', inset: 0, borderRadius: '50%',
        border: '1px solid rgba(212,175,55,0.18)',
        animation: `sqiPulse ${2+i*0.4}s ease-out infinite`,
        animationDelay: `${i*0.3}s`,
      }} />
    ))}
    <div style={{ position: 'absolute', inset: '20%', borderRadius: '50%', background: 'radial-gradient(circle,rgba(212,175,55,0.2) 0%,transparent 70%)', animation: 'sqiGlow 3s ease-in-out infinite alternate' }} />
    <div style={{ position: 'absolute', inset: '35%', borderRadius: '50%', background: 'radial-gradient(circle,rgba(212,175,55,0.5) 0%,rgba(212,175,55,0.1) 100%)', boxShadow: '0 0 20px rgba(212,175,55,0.3)' }} />
  </div>
);
...
// LOCK GATE
// ─────────────────────────────────────────────────────────────────────────────
const LockGate: React.FC<{ tier: TierSlug }> = ({ tier }) => {
  const navigate = useNavigate();
  const cfg = TIER_CONFIG[tier];
  return (
    <div style={{ background: 'rgba(5,5,5,0.9)', backdropFilter: 'blur(40px)', border: `1px solid ${cfg.color}22`, borderRadius: 24, padding: '40px 32px', textAlign: 'center' }}>
      <Lock size={32} color={cfg.color} style={{ margin: '0 auto 16px', opacity: 0.7, display: 'block' }} />
      <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.4em', color: cfg.color, textTransform: 'uppercase', marginBottom: 12, opacity: 0.7 }}>{cfg.label} Required</div>
      <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)', lineHeight: 1.6, marginBottom: 28 }}>
        This module is available from the <strong style={{ color: cfg.color }}>{cfg.label}</strong> tier ({cfg.price}).
      </p>
      <button onClick={() => navigate(cfg.upgradeUrl)} style={{
        background: `rgba(212,175,55,0.1)`, border: `1px solid ${cfg.color}66`, borderRadius: 14, padding: '12px 28px',
        color: cfg.color, fontSize: 10, fontWeight: 800, letterSpacing: '0.3em', textTransform: 'uppercase', cursor: 'pointer',
        boxShadow: `0 0 20px ${cfg.glow}`,
      }}>Upgrade to {cfg.label} ↗</button>
    </div>
  );
};
...
const HolyScienceCurriculum: React.FC = () => {
  ...
  const userRank = getTierRank(status.tier);
  const canAccess = (tier: TierSlug): boolean => status.isAdmin || userRank >= TIER_CONFIG[tier].rank;
  const tierOrder: TierSlug[] = ['free', 'prana-flow', 'siddha-quantum', 'akasha-infinity'];
  ...
  if (selectedLesson && selectedModule) {
    const tierCfg = TIER_CONFIG[selectedLesson.tier];
    return (
      <div style={{ minHeight: '100vh', background: '#050505', color: 'white', fontFamily: "'Plus Jakarta Sans','Inter',sans-serif", padding: '24px 20px 80px' }}>
        <style>{CSS_ANIM}</style>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <button onClick={() => setSelectedLesson(null)} style={backBtnStyle}>
            <ArrowLeft size={14} /> Back to {selectedModule.title}
          </button>
          <div style={{ marginBottom: 32 }}>
            <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: '0.5em', color: `${tierCfg.color}88`, textTransform: 'uppercase', marginBottom: 8 }}>
              Module {selectedModule.number} · {tierCfg.label}
            </div>
            <h1 style={{ fontSize: 'clamp(20px,4vw,30px)', fontWeight: 900, letterSpacing: '-0.04em', color: '#D4AF37', textShadow: '0 0 20px rgba(212,175,55,0.3)', margin: '0 0 8px', lineHeight: 1.1 }}>
              {selectedLesson.title}
            </h1>
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', fontWeight: 700 }}>{selectedLesson.duration}</div>
          </div>
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(212,175,55,0.08)', borderRadius: 24, padding: '36px', marginBottom: 20 }}>
            <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.72)', lineHeight: 1.9, whiteSpace: 'pre-wrap' }}>
              {selectedLesson.content}
            </div>
          </div>
          {selectedLesson.revelation && (
            <div style={{ background: 'rgba(212,175,55,0.04)', border: '1px solid rgba(212,175,55,0.18)', borderRadius: 20, padding: '28px 32px', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: '0.5em', color: 'rgba(212,175,55,0.5)', textTransform: 'uppercase', marginBottom: 14 }}>◈ Akasha-Archive Direct Transmission</div>
              <p style={{ fontSize: 13, color: 'rgba(212,175,55,0.85)', fontStyle: 'italic', lineHeight: 1.75, margin: 0 }}>{selectedLesson.revelation}</p>
            </div>
          )}
        </div>
      </div>
  ...
  // ── MODULE VIEW ───────────────────────────────────────────────────────────
  if (selectedModule) {
    const access = canAccess(selectedModule.tier);
    const tierCfg = TIER_CONFIG[selectedModule.tier];
    return (
      <div style={{ minHeight: '100vh', background: '#050505', color: 'white', fontFamily: "'Plus Jakarta Sans','Inter',sans-serif", padding: '24px 20px 80px' }}>
        <style>{CSS_ANIM}</style>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <button onClick={() => setSelectedModule(null)} style={backBtnStyle}>
            <ArrowLeft size={14} /> All Modules
          </button>
          <div style={{ marginBottom: 36 }}>
            <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: '0.5em', color: `${tierCfg.color}88`, textTransform: 'uppercase', marginBottom: 10 }}>
              Module {selectedModule.number} · {tierCfg.label}
            </div>
            <h1 style={{ fontSize: 'clamp(22px,4vw,34px)', fontWeight: 900, letterSpacing: '-0.04em', color: '#D4AF37', textShadow: '0 0 20px rgba(212,175,55,0.3)', margin: '0 0 8px', lineHeight: 1.1 }}>
              {selectedModule.title}
            </h1>
            <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.35)', margin: '0 0 4px' }}>{selectedModule.subtitle}</p>
            <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.3em', color: `${tierCfg.color}66`, textTransform: 'uppercase' }}>
              {selectedModule.transmissionType} · {selectedModule.totalDuration}
            </div>
          </div>
          {!access
            ? <LockGate tier={selectedModule.tier} />
            : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {selectedModule.lessons.map((lesson, i) => (
                  <button key={lesson.id} onClick={() => setSelectedLesson(lesson)}
                    style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(212,175,55,0.08)', borderRadius: 18, padding: '20px 24px', textAlign: 'left', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: 16 }}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(212,175,55,0.25)'; (e.currentTarget as HTMLElement).style.transform = 'translateX(4px)'; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(212,175,55,0.08)'; (e.currentTarget as HTMLElement).style.transform = 'translateX(0)'; }}
                  >
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(212,175,55,0.08)', border: '1px solid rgba(212,175,55,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <Play size={14} color="#D4AF37" />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.85)', marginBottom: 3, lineHeight: 1.3 }}>
                        {String(i + 1).padStart(2, '0')}. {lesson.title}
                      </div>
                      <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', fontWeight: 700 }}>{lesson.duration}</div>
                    </div>
                    <CheckCircle size={16} color="rgba(212,175,55,0.3)" />
                  </button>
                ))}
              </div>
            )
  ...
  // ── MAIN INDEX ────────────────────────────────────────────────────────────
  return (
    <div style={{ minHeight: '100vh', background: '#050505', color: 'white', fontFamily: "'Plus Jakarta Sans','Inter',sans-serif", padding: '0 0 80px' }}>
      <style>{CSS_ANIM}</style>
      {/* HERO */}
      <div style={{ textAlign: 'center', padding: '52px 24px 40px' }}>
        <ScalarOrb size={140} />
        <div style={{ marginTop: 28 }}>
          <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: '0.55em', color: 'rgba(212,175,55,0.38)', textTransform: 'uppercase', marginBottom: 12 }}>
            SQI Academy · Siddha Quantum Intelligence
          </div>
          <h1 style={{ fontSize: 'clamp(28px,6vw,52px)', fontWeight: 900, letterSpacing: '-0.05em', background: 'linear-gradient(135deg,#D4AF37,rgba(212,175,55,0.55))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', margin: '0 0 12px', lineHeight: 1.05 }}>
            Kaivalya Darsanam
          </h1>
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.32)', maxWidth: 440, margin: '0 auto 20px', lineHeight: 1.7 }}>
            The Holy Science of Sri Yukteshwar — 8 Modules across 4 Initiation Tiers with Direct Scalar Transmission
          </p>
          {/* Tier badges */}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center' }}>
            {tierOrder.map(t => {
              const cfg = TIER_CONFIG[t];
              const active = canAccess(t);
              return (
                <div key={t} style={{
                  ...
                  color: active ? cfg.color : 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', gap: 5,
                }}>
                  {active ? <CheckCircle size={8} /> : <Lock size={8} />}
                  {cfg.label}
                </div>
              );
            })}
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 20px' }}>
        {tierOrder.map(tierSlug => {
          const tierMods = CURRICULUM.filter(m => m.tier === tierSlug);
          const cfg = TIER_CONFIG[tierSlug];
          const hasAccess = canAccess(tierSlug);
          return (
            <div key={tierSlug} style={{ marginBottom: 36 }}>
              {/* Tier header */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14, paddingBottom: 12, borderBottom: `1px solid ${cfg.color}18` }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: cfg.color, boxShadow: `0 0 8px ${cfg.glow}`, flexShrink: 0 }} />
                <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.45em', color: cfg.color, textTransform: 'uppercase' }}>
                  {cfg.label} {hasAccess ? '· Unlocked' : `· ${cfg.price}`}
                </div>
                {!hasAccess && (
                  <button onClick={() => navigate(cfg.upgradeUrl)} style={{
                    marginLeft: 'auto', background: 'none', border: `1px solid ${cfg.color}44`,
                    borderRadius: 10, padding: '4px 12px', color: cfg.color,
                    fontSize: 8, fontWeight: 800, letterSpacing: '0.25em', textTransform: 'uppercase', cursor: 'pointer',
                  }}>Upgrade ↗</button>
                )}
              </div>
              {/* Modules */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {tierMods.map(mod => {
                  ...
                  const access = canAccess(mod.tier);
                  return (
                    <div key={mod.id} style={{
                      background: 'rgba(255,255,255,0.02)', backdropFilter: 'blur(20px)',
                      border: `1px solid ${access ? 'rgba(212,175,55,0.1)' : 'rgba(255,255,255,0.04)'}`,
                      borderRadius: 20, overflow: 'hidden', transition: 'border-color 0.2s',
                    }}>
                      <button
                        onClick={() => access ? toggleModule(mod.id) : toast.info(`Upgrade to ${cfg.label} to unlock this module`)}
                      ...
                      >
                        <div style={{ width: 40, height: 40, borderRadius: 12, flexShrink: 0, background: access ? 'rgba(212,175,55,0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${access ? 'rgba(212,175,55,0.2)' : 'rgba(255,255,255,0.05)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 900, color: access ? '#D4AF37' : 'rgba(255,255,255,0.2)' }}>
                          {access ? mod.number : <Lock size={14} color="rgba(255,255,255,0.2)" />}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: 700, color: access ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.3)', marginBottom: 3, lineHeight: 1.3 }}>{mod.title}</div>
                          <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.25em', color: access ? `${cfg.color}88` : 'rgba(255,255,255,0.15)', textTransform: 'uppercase' }}>
                            {mod.transmissionType} · {mod.totalDuration}
                          </div>
                        </div>
                        {access && (isExpanded ? <ChevronUp size={16} color="rgba(212,175,55,0.4)" /> : <ChevronDown size={16} color="rgba(255,255,255,0.2)" />)}
                      </button>
                      {isExpanded && access && (
                        <div style={{ borderTop: '1px solid rgba(212,175,55,0.06)', padding: '8px 14px 14px' }}>
                          {mod.lessons.map((lesson, i) => (
                            <button key={lesson.id}
                              onClick={() => { setSelectedModule(mod); setSelectedLesson(lesson); }}
                              style={{ width: '100%', background: 'none', border: 'none', padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', borderRadius: 12, textAlign: 'left', transition: 'background 0.15s' }}
                              onMouseEnter={e => (e.currentTarget.style.background = 'rgba(212,175,55,0.05)')}
                              onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                            >
                              <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'rgba(212,175,55,0.3)', flexShrink: 0 }} />
                              <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', flex: 1, lineHeight: 1.3 }}>{String(i+1).padStart(2,'0')}. {lesson.title}</span>
                              <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.2)', fontWeight: 700, flexShrink: 0 }}>{lesson.duration}</span>
                            </button>
                          ))}
                          <button onClick={() => setSelectedModule(mod)} style={{ marginTop: 8, width: '100%', background: 'rgba(212,175,55,0.06)', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 12, padding: '10px', color: '#D4AF37', fontSize: 9, fontWeight: 800, letterSpacing: '0.3em', textTransform: 'uppercase', cursor: 'pointer' }}>
                            Enter Module →
                          </button>
                        </div>
                      )}
      ...
      {/* FOOTER */}
      <div style={{ textAlign: 'center', padding: '40px 20px 0', borderTop: '1px solid rgba(212,175,55,0.04)', marginTop: 20 }}>
        <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: '0.5em', color: 'rgba(212,175,55,0.15)', textTransform: 'uppercase' }}>
          Scalar Transmission Active · Anahata Activation Engaged · Siddha Quantum Intelligence 2050
        </div>
      </div>
    </div>
  );
...
export default HolyScienceCurriculum;
