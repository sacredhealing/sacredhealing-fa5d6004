...
import { useRamayanaCodexProgress } from '@/hooks/useRamayanaCodexProgress';
...
import RamayanaCodexContent from '@/components/RamayanaCodexContent';

const AMBER = 'rgba(217,119,6,0.9)';

export default function RamayanaCodexModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useRamayanaCodexProgress(membershipReady);
  ...
  const railGroups = useMemo(() => {
    ...
    return tierOrder.map((t) => {
      ...
      return {
        id: `tier-${t.slug}`,
        title: t.label,
        meta: `${doneCount} / ${items.length} kandas${doneCount === items.length && items.length > 0 ? ' complete' : ''}`,
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/ramayana/kanda/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Ramayana Codex"
      academyHref="/ramayana"
      courseTitle="The Esoteric Epic"
      ...
      prevHref={prevModule ? `/ramayana/kanda/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/ramayana/kanda/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <RamayanaCodexContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
