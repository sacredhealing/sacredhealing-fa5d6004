...
import PujaEducationContent from '@/components/PujaEducationContent';

const AMBER = 'rgba(217,119,6,0.9)';

export default function PujaEducationModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = usePujaEducationProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/puja-education/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      ...
      academyHref="/puja-education"
      courseTitle="The Puja Hall"
      ...
      prevHref={prevModule ? `/puja-education/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/puja-education/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <PujaEducationContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
