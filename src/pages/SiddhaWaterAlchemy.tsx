...
export default function SiddhaWaterAlchemy() {
  ...
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = useWaterAlchemyProgress(membershipReady);
    ...
    <div style={{ minHeight: '100vh', background: '#050505', color: 'rgba(255,255,255,0.9)', paddingBottom: 104 }}>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        <CourseSyllabus
          ...
          courseTitle="Sacred Water Alchemy"
          academyName="Sacred Water Alchemy"
          progressLabel={`${stats.completedModules} / ${courses.length || 40} · ${stats.completionPercent}%`}
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/sacred-water/module/${lessonId}`);
            }
          }}
        />
        ...
        <footer style={{ marginTop: 40, paddingBottom: 32, textAlign: 'center' }}>
          <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.45em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)' }}>
            Neer · The Living Consciousness of Water
          </p>
        </footer>
      </div>
    </div>
