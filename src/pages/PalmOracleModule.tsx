...
import PalmOracleContent from '@/components/PalmOracleContent';

const VIOLET = 'rgba(168,85,247,0.9)';

export default function PalmOracleModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = usePalmOracleProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/palm-oracle/module/${c.id}` };
        }),
      };
    });
  ...
  if (!membershipReady || !module) {
    return (
      <div style={{ minHeight: '100vh', background: '#050505', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 36, height: 36, borderRadius: '50%', border: '2px solid rgba(168,85,247,.2)', borderTopColor: '#A855F7', animation: 'spin 0.8s linear infinite' }} />
      </div>
    );
  }
  ...
  return (
    <ModuleReaderShell
      ...
      academyHref="/palm-oracle"
      courseTitle="Hasta Samudrika Shastra"
      ...
      prevHref={prevModule ? `/palm-oracle/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/palm-oracle/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <PalmOracleContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
