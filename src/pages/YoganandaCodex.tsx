...
import { violet } from '@/components/education/tokens';
...
export default function YoganandaCodex() {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { isAdmin } = useAdminRole();
  const { tier, loading: membershipLoading, settled } = useMembership();
  const membershipReady = !membershipLoading && settled;
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = useYoganandaCodexProgress(membershipReady);
  ...
  const syllabusGroups = useMemo(() => {
    return TIER_ORDER.map((t) => {
      ...
      return {
        id: `tier-${t.slug}`,
        title: t.label,
        meta: `${completed} / ${mods.length} chapters${completed === mods.length && mods.length > 0 ? ' complete' : ''}`,
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050505', color: 'rgba(255,255,255,0.9)', paddingBottom: 104 }}>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        >
          <ArrowLeft size={14} color="rgba(167,139,250,.8)" /> Back
        </button>
        ...
        <CourseSyllabus
          ...
          courseTitle="Yogananda Codex"
          academyName="Yogananda Codex"
          progressLabel={`${stats.completedModules} / ${courses.length || 49} · ${stats.completionPercent}%`}
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/yogananda-codex/chapter/${lessonId}`);
            }
          }}
        />
        ...
        <footer style={{ marginTop: 40, paddingBottom: 32, textAlign: 'center' }}>
          <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.45em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)' }}>
            Autobiography of a Yogi — Decoded Through the Siddha Lens
          </p>
        </footer>
      </div>
    </div>
