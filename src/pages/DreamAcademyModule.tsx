...
import { useDreamAcademyProgress } from '@/hooks/useDreamAcademyProgress';
...
import DreamAcademyContent from '@/components/DreamAcademyContent';

const VIOLET = 'rgba(167,139,250,0.9)';

export default function DreamAcademyModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useDreamAcademyProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/dream-academy/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Svapna Vidyā"
      academyHref="/dream-academy"
      courseTitle="The Dream Stream"
      ...
      prevHref={prevModule ? `/dream-academy/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/dream-academy/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <DreamAcademyContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
