...
import { useAbundanceSadhanaProgress } from '@/hooks/useAbundanceSadhanaProgress';
...
import AbundanceSadhanaContent from '@/components/AbundanceSadhanaContent';

export default function AbundanceSadhanaModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useAbundanceSadhanaProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/abundance-curriculum/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Abundance Sadhana"
      academyHref="/abundance-curriculum"
      courseTitle="The Wealth Transmission"
      ...
      prevHref={prevModule ? `/abundance-curriculum/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/abundance-curriculum/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <AbundanceSadhanaContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
