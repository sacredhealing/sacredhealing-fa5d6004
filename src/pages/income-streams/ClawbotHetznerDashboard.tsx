// CLAWBOT — Hetzner Live Dashboard
// Mirrors PolymarketBotDetail.tsx exactly, route: /income-streams/clawbot-hetzner
...
// ── Server badge showing Hetzner origin ────────────────────────────────────
function HetznerBadge() {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.2em', color: 'rgba(212,175,55,0.8)', textTransform: 'uppercase' }}>
        HETZNER · 178.105.183.74
      </span>
    </div>
  );
}

export default function ClawbotHetznerDashboard() {
  ...
  const loadMembership = useCallback(async () => {
    ...
    const { data: myTradesData } = await sb
    ...
    const { data: fees } = await sb
      ...
      .eq('user_id', user.id);
    if (fees?.length) {
      const totalGross = fees.reduce((s: number, f: any) => s + parseFloat(f.gross_pnl_usdc || 0), 0);
      const totalFees = fees.reduce((s: number, f: any) => s + parseFloat(f.fee_usdc || 0), 0);
      const totalNet = fees.reduce((s: number, f: any) => s + parseFloat(f.net_pnl_usdc || 0), 0);
        ...
        <div className="mb-5 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              ...
              <Pill>Polymarket Whale Mirror</Pill>
            </div>
            ...
            <div className={`${GLASS} p-5 mb-4`}>
              ...
              {trades.length === 0 ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-xs text-white/25">Scanning Polymarket for whale signals…</p>
                </div>
