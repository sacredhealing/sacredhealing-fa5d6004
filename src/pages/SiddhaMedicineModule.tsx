...
import SiddhaMedicineModuleContent from '@/components/SiddhaMedicineModuleContent';

const TIER_SLUG_TO_CURRICULUM_KEY: Record<string, string> = {
  free: 'free',
  'prana-flow': 'prana',
  'siddha-quantum': 'quantum',
  'akasha-infinity': 'akasha',
};

export default function SiddhaMedicineModule() {
  ...
  const { courses, progressByModuleId, markComplete, stats } = useSiddhaMedicineProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/siddha-medicine/module/${c.id}` };
        }),
      };
    });
  ...
  const curriculumTier = TIER_SLUG_TO_CURRICULUM_KEY[module.tier_required || 'free'] || 'free';

  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Siddha Medicine Academy"
      academyHref="/siddha-medicine"
      courseTitle="Siddha Medicine Path"
      ...
      prevHref={prevModule ? `/siddha-medicine/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/siddha-medicine/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <SiddhaMedicineModuleContent tier={curriculumTier} moduleKey={module.module_key} moduleId={module.id} />
        ) : null
      }
    />
  );
