...
import { useNarasimhaProgress } from '@/hooks/useNarasimhaProgress';
...
import NarasimhaContent from '@/components/NarasimhaContent';

const AMBER = 'rgba(255,107,53,0.9)';

export default function NarasimhaModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useNarasimhaProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/narasimha/module/${c.id}` };
        }),
      };
    });
  ...
  if (!membershipReady || !module) {
    return (
      <div style={{ minHeight: '100vh', background: '#050505', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 36, height: 36, borderRadius: '50%', border: '2px solid rgba(255,107,53,.2)', borderTopColor: '#FF6B35', animation: 'spin 0.8s linear infinite' }} />
      </div>
    );
  }
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Narasimha Sacred Path"
      academyHref="/narasimha"
      courseTitle="The Nine Seals"
      courseIcon={<Sun size={24} />}
      ...
      prevHref={prevModule ? `/narasimha/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/narasimha/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <NarasimhaContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
