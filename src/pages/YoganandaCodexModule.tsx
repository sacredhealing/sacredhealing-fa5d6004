...
import { useYoganandaCodexProgress } from '@/hooks/useYoganandaCodexProgress';
...
import { violet, fade } from '@/components/education/tokens';
import YoganandaCodexContent from '@/components/YoganandaCodexContent';

export default function YoganandaCodexModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useYoganandaCodexProgress(membershipReady);
  ...
  const railGroups = useMemo(() => {
    ...
    return tierOrder.map((t) => {
      ...
      return {
        id: `tier-${t.slug}`,
        title: t.label,
        meta: `${doneCount} / ${items.length} chapters${doneCount === items.length && items.length > 0 ? ' complete' : ''}`,
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/yogananda-codex/chapter/${c.id}` };
        }),
      };
    });
  ...
  const accent = violet(0.9);
  const salesHref = getSalesPageForRank(getCourseTierRequiredRank(module.tier_required));

  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Yogananda Codex"
      academyHref="/yogananda-codex"
      courseTitle="Autobiography Decoded"
      courseIcon={<Scroll size={24} />}
      ...
      prevHref={prevModule ? `/yogananda-codex/chapter/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/yogananda-codex/chapter/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <YoganandaCodexContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
