...
import YagnaContent from '@/components/YagnaContent';

const AMBER = 'rgba(217,119,6,0.9)';

export default function YagnaModuleReader() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useYagnaProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/yagna/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      ...
      academyHref="/yagna"
      courseTitle="The Fire Transmission"
      ...
      prevHref={prevModule ? `/yagna/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/yagna/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <YagnaContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
