...
import NadiLeafContent from '@/components/NadiLeafContent';

export default function NadiLeafModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useNadiLeafProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/nadi-leaf/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      ...
      academyHref="/nadi-leaf"
      courseTitle="Agastya Muni's Living Library"
      ...
      prevHref={prevModule ? `/nadi-leaf/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/nadi-leaf/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <NadiLeafContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
