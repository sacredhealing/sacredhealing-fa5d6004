...
import { useAlchemicalShivaProgress } from '@/hooks/useAlchemicalShivaProgress';
...
import AlchemicalShivaContent from '@/components/AlchemicalShivaContent';

const RED = 'rgba(255,107,107,0.9)';

export default function AlchemicalShivaModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useAlchemicalShivaProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/shiva-lingam/module/${c.id}` };
        }),
      };
    });
  ...
  if (!membershipReady || !module) {
    return (
      <div style={{ minHeight: '100vh', background: '#050505', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 36, height: 36, borderRadius: '50%', border: '2px solid rgba(255,107,107,.2)', borderTopColor: '#FF6B6B', animation: 'spin 0.8s linear infinite' }} />
      </div>
    );
  }
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Alchemical Shiva"
      academyHref="/shiva-lingam"
      courseTitle="The Nath Siddha Path"
      ...
      prevHref={prevModule ? `/shiva-lingam/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/shiva-lingam/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <AlchemicalShivaContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
