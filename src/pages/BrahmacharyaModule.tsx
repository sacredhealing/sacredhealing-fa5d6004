...
import BrahmacharyaContent from '@/components/BrahmacharyaContent';

const TEAL = 'rgba(20,184,166,0.9)';

export default function BrahmacharyaModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useBrahmacharyaProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/brahmacharya-academy/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      ...
      academyHref="/brahmacharya-academy"
      courseTitle="Ojas · Tejas · Prana"
      ...
      prevHref={prevModule ? `/brahmacharya-academy/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/brahmacharya-academy/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <BrahmacharyaContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
