...
import MediumshipAcademyContent from '@/components/MediumshipAcademyContent';

const VIOLET = 'rgba(167,139,250,0.9)';

export default function MediumshipAcademyModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useMediumshipAcademyProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/siddha-mediumship-academy/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      ...
      academyHref="/siddha-mediumship-academy"
      courseTitle="The Akasha Transmission"
      ...
      prevHref={prevModule ? `/siddha-mediumship-academy/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/siddha-mediumship-academy/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <MediumshipAcademyContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
