...
import ShaktiCycleContent from '@/components/ShaktiCycleContent';

const ROSE = 'rgba(225,29,143,0.9)';

export default function ShaktiCycleModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useShaktiCycleProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/sovereign-hormonal-alchemy/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      ...
      academyHref="/sovereign-hormonal-alchemy"
      courseTitle="Shakti Cycle Intelligence"
      ...
      prevHref={prevModule ? `/sovereign-hormonal-alchemy/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/sovereign-hormonal-alchemy/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <ShaktiCycleContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
