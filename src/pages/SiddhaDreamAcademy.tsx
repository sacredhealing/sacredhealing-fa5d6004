...
export default function SiddhaDreamAcademy() {
  ...
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = useDreamAcademyProgress(membershipReady);
    ...
    <div style={{ minHeight: '100vh', background: '#050505', color: 'rgba(255,255,255,0.9)', paddingBottom: 104 }}>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        <CourseSyllabus
          ...
          courseTitle="Svapna Vidyā"
          academyName="Svapna Vidyā"
          progressLabel={`${stats.completedModules} / ${courses.length || 15} · ${stats.completionPercent}%`}
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/dream-academy/module/${lessonId}`);
            }
          }}
        />
        ...
        <footer style={{ marginTop: 40, paddingBottom: 32, textAlign: 'center' }}>
          <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.45em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)' }}>
            The Complete Siddha Dream Science
          </p>
        </footer>
      </div>
    </div>
