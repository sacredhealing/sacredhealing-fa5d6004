...
/** Map legacy / Temple registry IDs to this page's `SITES` rows */
function resolveSite(siteId: string | undefined): (typeof SITES)[0] | null {
  if (!siteId) return null;
  const direct = SITES.find((s) => s.id === siteId);
  if (direct) return direct;
  const aliases: Record<string, string> = {
    kailash_13x: 'kailash',
    machu_picchu: 'machu',
    vrindavan_krsna: 'anc_vrindavan',
    ayodhya_rama: 'anc_ayodhya',
  };
  const alt = aliases[siteId];
  return alt ? SITES.find((s) => s.id === alt) ?? null : null;
}

function strengthDescFor(strength: number): string {
  const tier = ([90, 75, 55, 35, 15, 0] as const).find((t) => strength >= t) ?? 0;
  return STR_DESCS[tier as keyof typeof STR_DESCS] ?? STR_DESCS[0];
}
...
export default function VirtualPilgrimage() {
  ...
  useEffect(() => {
    if (activation?.strength != null) setStrengthLocal(activation.strength);
  }, [activation?.strength]);
  ...
  const activeSite = activation ? resolveSite(activation.siteId) : null;
  ...
  async function handleActivate(){
    ...
    if (!user) {
      toast.error('Sign in to lock a pilgrimage to your account.');
      return;
    }
    try {
      await activateSite({
      ...
      toast.success(`⟁ Locked — ${previewSite.nm}`);
      setPreviewSite(null);
      setDdOpen(false);
  ...
  if (!user) {
    return (
      <div style={{ ...S.root, padding: '48px 20px', textAlign: 'center' }}>
        <div style={{ color: '#D4AF37', fontSize: 22, fontWeight: 900, marginBottom: 12 }}>Virtual Pilgrimage</div>
        <p style={{ color: 'rgba(255,255,255,.55)', fontSize: 14, lineHeight: 1.75, maxWidth: 400, margin: '0 auto 28px' }}>
          Sign in to save your home GPS and pilgrimage lock to Supabase so the hourly Railway pulse keeps your field registered when devices are off.
        </p>
        <Link
          to="/auth"
          style={{
            ...
            padding: '14px 28px',
            borderRadius: 999,
            background: 'linear-gradient(135deg,#D4AF37,#8B7A28)',
            color: '#050505',
            fontWeight: 800,
            fontSize: 12,
            letterSpacing: '.15em',
        ...
        <div style={{ marginTop: 24 }}>
          <Link to="/temple-home" style={{ color: 'rgba(212,175,55,.65)', fontSize: 12, fontWeight: 700 }}>
            ← Temple Home
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={S.root}>
      ...
      {/* ── HEADER ── */}
      <div style={{padding:"12px 16px 0",display:"flex",justifyContent:"space-between",alignItems:"center",maxWidth:520,margin:"0 auto"}}>
        <Link to="/temple-home" style={{color:"rgba(212,175,55,.75)",fontSize:11,fontWeight:800,textDecoration:"none",letterSpacing:".08em"}}>← Temple</Link>
        <span style={{fontSize:9,color:"rgba(255,255,255,.25)",letterSpacing:".25em"}}> /virtual-pilgrimage</span>
      </div>
      ...
      <div style={{padding:"0 16px"}}>
            ...
            {activeTab==="practice" && (
              <div>
                ...
                <div style={{display:"flex",gap:8,marginBottom:14}}>
                  <button disabled={curStep===0} onClick={()=>setCurStep(s=>Math.max(0,s-1))} style={{...S.btn,flex:1,background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.1)",color:"rgba(255,255,255,.45)",opacity:curStep===0?0.4:1}}>← Back</button>
        ...
        {activation && !activeSite && (
          <div style={{ ...S.card, marginTop: 12 }}>
            <span style={S.lbl}>Active lock · expanded UI pending</span>
            <p style={{ color: 'rgba(255,255,255,.82)', fontSize: 15, fontWeight: 800, marginBottom: 8 }}>{activation.siteName}</p>
            <p style={{ color: 'rgba(255,255,255,.45)', fontSize: 12, lineHeight: 1.7, marginBottom: 14 }}>
              Site ID <code style={{ color: '#D4AF37' }}>{activation.siteId}</code> is not mapped to the full 40+ portal ritual pack yet. Your scalar bridge from Supabase is still shown below — release to choose a listed site.
            </p>
            {scalar && (
              <div style={{ ...S.scalarRow, flexDirection: 'column', alignItems: 'flex-start', gap: 6, marginBottom: 14 }}>
                <span style={{ color: '#D4AF37', fontWeight: 800 }}>
                  {scalar.carrierHz.toFixed(2)} Hz carrier · {scalar.binauralHz.toFixed(2)} Hz binaural · {scalar.bearingDir} {scalar.bearingDeg}° · {scalar.distanceKm.toLocaleString()} km
                </span>
              </div>
            )}
            <button type="button" onClick={() => void handleRelease()} style={{ ...S.btn, background: 'rgba(255,100,50,.12)', border: '1px solid rgba(255,100,50,.35)', color: '#FF8C00' }}>
              Release lock
            </button>
          </div>
        )}
