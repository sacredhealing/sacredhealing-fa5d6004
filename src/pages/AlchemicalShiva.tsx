...
export default function AlchemicalShiva() {
  ...
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = useAlchemicalShivaProgress(membershipReady);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050505', color: 'rgba(255,255,255,0.9)', paddingBottom: 104 }}>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        >
          <ArrowLeft size={14} color={RED} /> Back
        </button>
        ...
        {!user && (
          <div style={{ marginBottom: 32, textAlign: 'center' }}>
            <button
              ...
              style={{
                borderRadius: 999, padding: '14px 40px', background: 'linear-gradient(135deg,#FF6B6B,#C0392B)',
                border: 'none', color: '#050505', fontSize: 11, fontWeight: 800, letterSpacing: '.28em', textTransform: 'uppercase',
                cursor: 'pointer', boxShadow: '0 0 40px rgba(255,107,107,0.22)',
              }}
            >
              Begin Initiation
            </button>
        ...
        <CourseSyllabus
          ...
          courseTitle="Alchemical Shiva"
          academyName="Alchemical Shiva"
          progressLabel={`${stats.completedModules} / ${courses.length || 6} · ${stats.completionPercent}%`}
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/shiva-lingam/module/${lessonId}`);
            }
          }}
        />
        ...
        <footer style={{ marginTop: 40, paddingBottom: 32, textAlign: 'center' }}>
          <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.45em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)' }}>
            Nath Siddha Lineage — Gorakshanath's Transmission
          </p>
        </footer>
      </div>
    </div>
