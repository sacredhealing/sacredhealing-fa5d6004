...
import { gold } from '@/components/education/tokens';
...
export default function SiddhaQuantumMeditationCourse() {
  ...
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = useMeditationCourseProgress(membershipReady);
    ...
    return TIER_ORDER.map((t) => {
      ...
      return {
        ...
        lessons: mods.map((m) => {
          ...
          const allowed = hasFeatureAccess(isAdmin, tier, getCourseTierRequiredRank(m.tier_required), grantedAcademies.has('siddha_meditation_course'));
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050505', color: 'rgba(255,255,255,0.9)', paddingBottom: 104 }}>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        >
          <ArrowLeft size={14} color="rgba(212,175,55,.8)" /> Back
        </button>
        ...
        <CourseSyllabus
          ...
          courseTitle="Supreme Siddha Meditation"
          academyName="Supreme Siddha Meditation"
          progressLabel={`${stats.completedModules} / ${courses.length || 14} · ${stats.completionPercent}%`}
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/meditation-course/module/${lessonId}`);
            }
          }}
        />
        ...
        <footer style={{ marginTop: 40, paddingBottom: 32, textAlign: 'center' }}>
          <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.45em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)' }}>
            18 Siddha Masters · The Hall of Stillness
          </p>
        </footer>
      </div>
    </div>
