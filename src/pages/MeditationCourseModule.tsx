...
import { useMeditationCourseProgress } from '@/hooks/useMeditationCourseProgress';
...
import MeditationModuleContent from '@/components/MeditationModuleContent';

export default function MeditationCourseModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useMeditationCourseProgress(membershipReady);
  ...
  const railGroups = useMemo(() => {
    const tierOrder: { slug: string; label: string }[] = [
      { slug: 'free', label: 'Foundation of Stillness' },
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/meditation-course/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Supreme Siddha Meditation"
      academyHref="/meditation-course"
      courseTitle="The Hall of Stillness"
      ...
      prevHref={prevModule ? `/meditation-course/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/meditation-course/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <MeditationModuleContent moduleId={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
