...
import { useSoundAlchemyProgress } from '@/hooks/useSoundAlchemyProgress';
...
import SoundAlchemyContent from '@/components/SoundAlchemyContent';

const AMBER = 'rgba(217,119,6,0.9)';

export default function SoundAlchemyModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useSoundAlchemyProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/siddha-sound-alchemy/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Siddha Sound Alchemy"
      academyHref="/siddha-sound-alchemy"
      courseTitle="Nada Vijnana"
      courseIcon={<Waves size={24} />}
      ...
      prevHref={prevModule ? `/siddha-sound-alchemy/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/siddha-sound-alchemy/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <SoundAlchemyContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
