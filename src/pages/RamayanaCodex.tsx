...
export default function RamayanaCodex() {
  ...
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = useRamayanaCodexProgress(membershipReady);
  ...
  const syllabusGroups = useMemo(() => {
    return TIER_ORDER.map((t) => {
      ...
      return {
        id: `tier-${t.slug}`,
        title: t.label,
        meta: `${completed} / ${mods.length} kandas${completed === mods.length && mods.length > 0 ? ' complete' : ''}`,
    ...
    <div style={{ minHeight: '100vh', background: '#050505', color: 'rgba(255,255,255,0.9)', paddingBottom: 104 }}>
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        {!user && (
          <div style={{ marginBottom: 32, textAlign: 'center' }}>
            <button
              ...
              style={{
                borderRadius: 999, padding: '14px 40px', background: 'linear-gradient(135deg,#D97706,#92400E)',
        ...
        <CourseSyllabus
          ...
          courseTitle="Ramayana Codex"
          academyName="Ramayana Codex"
          progressLabel={`${stats.completedModules} / ${courses.length || 7} · ${stats.completionPercent}%`}
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/ramayana/kanda/${lessonId}`);
            }
          }}
        />
        ...
        <footer style={{ marginTop: 40, paddingBottom: 32, textAlign: 'center' }}>
          <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.45em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)' }}>
            The Esoteric Ramayana — 7 Kandas, 7 Chakras
          </p>
        </footer>
      </div>
    </div>
