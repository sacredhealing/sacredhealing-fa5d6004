...
/* ─── Scoped CSS (avoids global * / layout clashes under AppLayout + other pages) ─ */
const SHG_SCOPE = "shg-siddha-hair";
const SQI_CSS = `
  .${SHG_SCOPE}, .${SHG_SCOPE} * { box-sizing:border-box; }
  ...
  @keyframes shg-shg-sqi-pulse {
    0%   { transform:scale(0.65); opacity:0.65; }
    75%  { transform:scale(1.5); opacity:0; }
    100% { transform:scale(1.5); opacity:0; }
  }
  @keyframes shg-shg-sqi-scan {
    0%   { transform:translateY(-100%); }
    100% { transform:translateY(400%); }
  }
  @keyframes shg-shg-sqi-spin { from{transform:rotate(0deg);} to{transform:rotate(360deg);} }
  @keyframes shg-shg-sqi-blink { 0%,100%{opacity:1;} 50%{opacity:0.2;} }
  @keyframes shg-shg-sqi-bar {
  ...
  return (
    <>
      ...
      <svg className="absolute inset-0 z-0 pointer-events-none w-full h-full"
        ...
        <defs>
          <filter id="shgHairNadiBlur"><feGaussianBlur stdDeviation={active?"2.5":"1"} result="b"/>
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
        </defs>
        <g filter="url(#shgHairNadiBlur)" stroke={active?CYAN:GOLD} strokeWidth={active?"1.2":".6"} fill="none">
  ...
  const captureAndAnalyze = async () => {
    ...
    try {
      ...
      if(err instanceof Error && err.message==="NO_API_KEY") {
        ...
        setTimeout(()=>{
          setPhase("questionnaire");
          toast.message("🔑 VITE_GEMINI_API_KEY not set",{
...
export default function SiddhaHairGrowth() {
  ...
  useEffect(() => {
    if (!memberLoading && !hasFeatureAccess(isAdmin, tier, FEATURE_TIER.siddhaPortal)) {
      navigate("/siddha-quantum");
    }
  }, [isAdmin, tier, memberLoading, navigate]);
  ...
  return (
    ...
    >
      <style>{SQI_CSS}</style>
      <div className="absolute inset-0 z-0 min-h-full pointer-events-none" aria-hidden>
        <AkashaBackground active={!!scanResult}/>
      </div>

      <div style={{position:"relative",zIndex:2,maxWidth:520,margin:"0 auto",padding:"22px 16px 0"}}>
