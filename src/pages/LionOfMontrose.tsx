...
export default function LionOfMontrose() {
  ...
  const { courses, progressByModuleId, stats, loading: loadingData, error: loadError } = useNarasimhaProgress(membershipReady);
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px 0' }}>
        ...
        {!user && (
          <div style={{ marginBottom: 32, textAlign: 'center' }}>
            <button
              ...
              style={{
                borderRadius: 999, padding: '14px 40px', background: 'linear-gradient(135deg,#FF6B35,#C0392B)',
                border: 'none', color: '#050505', fontSize: 11, fontWeight: 800, letterSpacing: '.28em', textTransform: 'uppercase',
                cursor: 'pointer', boxShadow: '0 0 40px rgba(255,107,53,0.22)',
              }}
            >
              Begin Initiation
            </button>
        ...
        {loadError && (
          <div style={{
            ...
            <p style={{ fontSize: 13, fontWeight: 700, color: '#F87171', margin: '0 0 4px' }}>Could not load this path.</p>
        ...
        <CourseSyllabus
          ...
          courseTitle="Narasimha Sacred Path"
          academyName="Narasimha Sacred Path"
          progressLabel={`${stats.completedModules} / ${courses.length || 13} · ${stats.completionPercent}%`}
          ...
          onLessonClick={(lessonId, locked) => {
            if (locked) {
              ...
              navigate(`/narasimha/module/${lessonId}`);
            }
          }}
        />
        ...
        <footer style={{ marginTop: 40, paddingBottom: 32, textAlign: 'center' }}>
          <p style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.45em', textTransform: 'uppercase', color: 'rgba(255,255,255,.25)' }}>
            The Nine Seals of the Man-Lion
          </p>
        </footer>
      </div>
    </div>
