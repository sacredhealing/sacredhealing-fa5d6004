...
import BreatharianModuleContent from '@/components/BreatharianModuleContent';

export default function BreatharianModule() {
  ...
  const { courses, progressByModuleId, markComplete, touchAccessed, stats } = useBreatharianProgress(membershipReady);
          ...
          const state: 'done' | 'current' | 'available' | 'locked' = done
            ...
            : isCurrentModule
              ...
              : allowed
                ? 'available'
                : 'locked';
          return { id: c.id, number: c.module_number, title: c.title, state, href: `/breatharian-academy/module/${c.id}` };
        }),
      };
    });
  ...
  return (
    <ModuleReaderShell
      accent={accent}
      academyName="Breatharian Academy"
      academyHref="/breatharian-academy"
      courseTitle="Pranic Living Path"
      ...
      prevHref={prevModule ? `/breatharian-academy/module/${prevModule.id}` : null}
      nextHref={nextModule && nextAllowed ? `/breatharian-academy/module/${nextModule.id}` : null}
      footerExtra={
        user?.id ? (
          <BreatharianModuleContent moduleKey={module.module_key} dbModuleId={module.id} />
        ) : null
      }
    />
  );
