...
import { SACRED_GEOMETRY_MODULES } from '@/data/sacredGeometryModuleContent';
...
const SacredGeometryContent: React.FC<{ moduleKey: string; dbModuleId: string }> = ({ moduleKey, dbModuleId }) => {
  const gModule = SACRED_GEOMETRY_MODULES.find((m) => m.id === moduleKey);
  const [openIndex, setOpenIndex] = React.useState(0);
  const { rows: sectionRows, toggleSectionComplete, setSectionNotes } = useSectionProgress(dbModuleId, 'user_sacred_geometry_section_progress');
            ...
            {isOpen && (
              <div style={{ padding: '0 20px 24px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 18, flexWrap: 'wrap', gap: 10 }}>
                  ...
                  <button type="button" disabled={i === gModule.lessons.length - 1} onClick={() => setOpenIndex(Math.min(gModule.lessons.length - 1, i + 1))} style={{
                    background: i === gModule.lessons.length - 1 ? 'transparent' : 'rgba(212,175,55,0.14)',
                    border: `1px solid ${i === gModule.lessons.length - 1 ? 'rgba(255,255,255,0.1)' : 'rgba(212,175,55,0.4)'}`,
                    color: i === gModule.lessons.length - 1 ? 'rgba(255,255,255,0.25)' : GOLD,
                    padding: '9px 16px', borderRadius: 999, fontSize: 10.5, fontWeight: 700,
                    cursor: i === gModule.lessons.length - 1 ? 'default' : 'pointer',
                  }}>
                    {i === gModule.lessons.length - 1 ? 'Last lesson' : 'Next lesson →'}
                  </button>
                </div>
              </div>
            )}
...
export default SacredGeometryContent;
