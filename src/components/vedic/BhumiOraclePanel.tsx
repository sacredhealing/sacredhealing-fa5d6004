...
// Reads birth data from JyotishChamber state, no extra DB calls
...
const GRAHAS = [
  ...
  { id:'moon',    name:'Chandra', sym:'☽', color:'#C0C8E8', dhatu:'Artha',        meaning:'Emotional depth, healing capacity, public connection. The mind finds its sanctuary frequency. Ideal for Nada healing work and creative flow.', zones:['Scandinavia','Canada','N. Europe'] },
  ...
  { id:'mercury', name:'Budha',   sym:'☿', color:'#10B981', dhatu:'Artha',        meaning:'Communication mastery, teaching, trade, digital business. Your voice carries greatest power here. Publishing and speaking thrive.', zones:['UK / Ireland','Germany','Japan'] },
  ...
  { id:'venus',   name:'Shukra', sym:'♀', color:'#EC4899', dhatu:'Kama',         meaning:'Love awakens, artistic genius flows, beauty manifests. Relationships of profound resonance form here. Retreat, honeymoon, creative excellence.', zones:['France / Italy','Bali','Brazil'] },
  { id:'saturn',  name:'Shani',  sym:'♄', color:'#6366F1', dhatu:'Karma',        meaning:'Deep karmic testing. Discipline rewarded over time. Not for pleasure — for mastery. Sade Sati lessons intensify here.', zones:['Russia','Eastern Europe','Argentina'] },
  { id:'rahu',    name:'Rāhu',   sym:'☊', color:'#8B5CF6', dhatu:'Maya',         meaning:'Amplified ambition, innovation, boundary-breaking. Material success is possible — but use consciously. Technology and foreign cultures flourish.', zones:['USA','China','SE Asia'] },
  { id:'ketu',    name:'Ketu',   sym:'☋', color:'#F97316', dhatu:'Moksha',       meaning:'Past-life liberation codes. Tamil Siddha lineage magnetically activated. Psychic opening, detachment, and moksha acceleration.', zones:['S. India / Lanka','Tibet / Himalayas','Peru / Andes','Egypt'] },
];

const REGIONS = [
  { id:'scandinavia',   label:'Scandinavia',       x:47, y:13, w:8,  h:10, graha:'moon'    },
  { id:'uk',            label:'UK / Ireland',       x:42, y:17, w:5,  h:5,  graha:'mercury' },
  { id:'europe_north',  label:'N. Europe',          x:47, y:18, w:10, h:8,  graha:'moon'    },
  ...
  { id:'indonesia',     label:'Bali / Indonesia',   x:72, y:36, w:5,  h:4,  graha:'venus'   },
...
const TOP_ZONES = [
  { rank:1, label:'India / Nepal',      graha:'Guru ♃',          color:'#D4AF37', why:'Jupiter zone · Current Antardasha · Dharma + wealth at maximum' },
  { rank:2, label:'S. India / Tamil Nadu', graha:'Ketu ☋',       color:'#F97316', why:'Tamil Siddha vortex · Moksha codes · Past-life lineage Kshetra' },
  { rank:3, label:'Scandinavia',        graha:'Chandra ☽',       color:'#C0C8E8', why:'Birth zone · Healing sanctuary · Nada transmission clarity' },
  { rank:4, label:'Thailand / Bali',    graha:'Guru ♃ + Shukra ♀',color:'#EC4899',why:'Jupiter + Venus · Sacred retreat · Love and wisdom convergence' },
];
...
export const BhumiOraclePanel: React.FC<Props> = ({ birthData, ephemeris }) => {
  ...
  const getGraha = (grahaId: string) => GRAHAS.find(g => g.id === grahaId);
  const getRegionGraha = (r: typeof REGIONS[0]) => getGraha(r.graha);

  const isDashaGraha = (grahaId: string) =>
    grahaId === mahaLord.toLowerCase() || grahaId === antarLord.toLowerCase() ||
    grahaId === 'jupiter' || grahaId === 'venus'; // fallback for known dasha

  const regionOpacity = (r: typeof REGIONS[0]) => {
    const g = getRegionGraha(r);
    if (!g) return 0.2;
    if (activeGraha && g.id !== activeGraha) return 0.05;
    if (selected?.id === r.id || hovering === r.id) return 0.8;
    if (isDashaGraha(g.id)) return 0.45;
    return 0.22;
  };

  const selectedGraha = selected ? getRegionGraha(selected) : null;

  const G = (id: string) => GRAHAS.find(g => g.id === id)!;
  ...
  return (
    <div style={ss}>
      ...
      <div style={{ marginBottom:20 }}>
        ...
        <h2 style={{ margin:'0 0 4px', fontSize:'clamp(20px,4vw,30px)', fontWeight:900, letterSpacing:'-0.04em', color:'#D4AF37', textShadow:'0 0 20px rgba(212,175,55,0.3)' }}>
          Sacred Geography Reader
        </h2>
        <p style={{ margin:0, fontSize:12, color:'rgba(255,255,255,0.4)', lineHeight:1.5 }}>
          {birthData ? `${birthData.birth_name} · ${birthData.birth_date} · ${birthData.birth_place}` : 'Graha-Geographic Intelligence from the Akasha-Neural Archive'}
        </p>
      </div>
      ...
      <div style={{ display:'flex', gap:0, marginBottom:20, background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:50, padding:4, width:'fit-content' }}>
        {(['map','grahas','blueprint'] as const).map(t => (
          <button key={t} onClick={() => setSubTab(t)} style={{
            ...
            padding:'7px 18px', cursor:'pointer', fontSize:11, fontWeight: subTab===t ? 700 : 400,
        ...
        <div style={{ display:'flex', gap:20, flexWrap:'wrap' }}>
          ...
          <div style={{ flex:'1 1 500px' }}>
            ...
            <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:12 }}>
              <button onClick={() => setActiveGraha(null)} style={{ background:!activeGraha?'rgba(212,175,55,0.12)':'rgba(255,255,255,0.02)', border:`1px solid ${!activeGraha?'#D4AF37':'rgba(255,255,255,0.06)'}`, borderRadius:20, padding:'3px 12px', cursor:'pointer', color:!activeGraha?'#D4AF37':'rgba(255,255,255,0.3)', fontSize:10, fontWeight:600 }}>All</button>
              {GRAHAS.map(g => (
                <button key={g.id} onClick={() => setActiveGraha(activeGraha===g.id?null:g.id)} style={{ background:activeGraha===g.id?`${g.color}18`:'rgba(255,255,255,0.02)', border:`1px solid ${activeGraha===g.id?g.color:'rgba(255,255,255,0.06)'}`, borderRadius:20, padding:'3px 10px', cursor:'pointer', color:activeGraha===g.id?g.color:'rgba(255,255,255,0.3)', fontSize:10, fontWeight:activeGraha===g.id?700:400, transition:'all 0.2s' }}>
                  {g.sym} {g.name}
                </button>
              ))}
            </div>
            ...
            <div style={{ background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:20, overflow:'hidden' }}>
              <svg viewBox="0 0 100 60" style={{ width:'100%', display:'block' }}>
                ...
                {REGIONS.map(r => {
                  ...
                  const op = regionOpacity(r);
                  const isDasha = isDashaGraha(g.id);
                  return (
                    <g key={r.id}>
                      <rect x={r.x} y={r.y} width={r.w} height={r.h} rx="1.5" ry="1.5"
                        fill={g.color} opacity={op} style={{ cursor:'pointer', transition:'all 0.25s' }}
                        onClick={() => setSelected(selected?.id===r.id?null:r)}
                        onMouseEnter={() => setHovering(r.id)}
                        onMouseLeave={() => setHovering(null)}
                      />
                      {isDasha && (
                        <rect x={r.x-0.5} y={r.y-0.5} width={r.w+1} height={r.h+1} rx="2" ry="2"
                          fill="none" stroke={g.color} strokeWidth="0.4"
                          opacity={0.25 + 0.25*Math.sin(pulse*0.063*Math.PI)}
                        />
                      )}
                      {(hovering===r.id || selected?.id===r.id) && (
                        <text x={r.x+r.w/2} y={r.y+r.h/2+1} textAnchor="middle" fontSize="1.7" fill="white" fontWeight="700" style={{pointerEvents:'none'}}>{r.label}</text>
                      )}
                    </g>
                  );
                })}
                {/* Birth marker */}
                <circle cx="48.5" cy="15" r={0.8+0.4*Math.sin(pulse*0.063*Math.PI)} fill="#D4AF37" opacity="0.9"/>
                <circle cx="48.5" cy="15" r="2" fill="none" stroke="#D4AF37" strokeWidth="0.3" opacity={0.3+0.2*Math.sin(pulse*0.063*Math.PI)}/>
                <text x="50" y="14.5" fontSize="1.4" fill="#D4AF37" fontWeight="800" opacity="0.8">✦ Uddevalla</text>
                <text x="1" y="58.5" fontSize="1.1" fill="rgba(255,255,255,0.15)">Glowing borders = Dasha-activated zones · Click any region for reading</text>
              </svg>
            </div>
          ...
          <div style={{ flex:'1 1 260px', minWidth:240 }}>
            {selected && selectedGraha ? (
              <div style={{ background:'rgba(255,255,255,0.02)', border:`1px solid ${selectedGraha.color}30`, borderRadius:20, padding:22, boxShadow:`0 0 40px ${selectedGraha.color}0d` }}>
                ...
                <h3 style={{ margin:'0 0 3px', fontSize:22, fontWeight:900, letterSpacing:'-0.03em', color:selectedGraha.color }}>{selectedGraha.sym} {selected.label}</h3>
                ...
                <div style={{ background:`${selectedGraha.color}0c`, border:`1px solid ${selectedGraha.color}20`, borderRadius:12, padding:14, marginBottom:14 }}>
                  <div style={{ fontSize:10, color:'rgba(255,255,255,0.35)', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', marginBottom:6 }}>Graha Intelligence</div>
                ...
                <button onClick={()=>setSelected(null)} style={{ width:'100%', background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:10, padding:'8px', color:'rgba(255,255,255,0.35)', cursor:'pointer', fontSize:11, fontWeight:600 }}>← Clear</button>
              </div>
            ) : (
              <div style={{ background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:20, padding:24 }}>
                <div style={{ fontSize:8, letterSpacing:'0.35em', color:'rgba(212,175,55,0.5)', fontWeight:800, textTransform:'uppercase', marginBottom:8 }}>Top Sacred Zones</div>
                {TOP_ZONES.map(z => (
                ...
                <div style={{ fontSize:11, color:'rgba(255,255,255,0.3)', lineHeight:1.6, marginTop:4 }}>Click any zone on the map to receive your full Bhumi transmission.</div>
              </div>
            )}
          </div>
        </div>
      ...
      {subTab === 'grahas' && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:14 }}>
          {GRAHAS.map(g => (
            <div key={g.id} style={{ background:'rgba(255,255,255,0.02)', border:`1px solid ${g.color}20`, borderRadius:18, padding:18, boxShadow:isDashaGraha(g.id)?`0 0 24px ${g.color}12`:'none', position:'relative', overflow:'hidden' }}>
              {isDashaGraha(g.id) && (
        ...
        <div style={{ maxWidth:680 }}>
          <div style={{ background:'rgba(255,255,255,0.02)', border:'1px solid rgba(212,175,55,0.12)', borderRadius:20, padding:26, marginBottom:16 }}>
            ...
            <div style={{ fontSize:11, color:'rgba(255,255,255,0.4)', lineHeight:1.7, marginBottom:16 }}>
              Based on Pushya Nakshatra Pada 2 · Deity: Brihaspati · Lord: Shani · The Nourisher
            </div>
            {[
              { title:'Primary Kshetra — India / Tamil Nadu', color:'#D4AF37', text:'Pushya Nakshatra carries the Brihaspati-Shani axis. Your deepest spiritual initiations await in South India — Chidambaram, Tiruvannamalai, Rameswaram, and Kashi. These zones dissolve karmic density fastest and amplify lineage transmission exponentially.' },
              { title:'Healing Sanctuary — Scandinavia / Nordic', color:'#C0C8E8', text:'Your Chandra in Scandinavia creates a sanctuary field. This is where your nervous system resets, healing transmissions amplify, and Nada Yoga work flows most purely. Birth zone resonance never fades — return here for restoration.' },
              { title:'Dharmic Wealth — Jupiter Zones', color:'#D4AF37', text:'Under your current Jupiter Antardasha, USA West Coast, Hawaii, Nepal, and Bali become portals for material expansion aligned with dharma. These are not pleasure zones alone — they are dharmic prosperity activators where guru energies reward aligned service.' },
              { title:'Moksha Gateway — S. India, Peru, Egypt', color:'#F97316', text:'Ketu zones carry past-life liberation codes. Short pilgrimage visits during Ketu transits (especially to Rameshwaram or Mahabalipuram) can accelerate liberation rapidly. Peru Andes and Egypt Nile axis carry parallel Akashic frequency.' },
              { title:'Creative Power — France, Bali, Brazil', color:'#EC4899', text:'Venus zones for artistic genius, love, and beauty creation. Ideal for music recording, creative retreats, and attracting high-resonance relationships. Your Nada healing work produced in Bali carries maximum Venus-Jupiter blessing.' },
            ].map(item => (
          ...
          <div style={{ background:'rgba(212,175,55,0.04)', border:'1px solid rgba(212,175,55,0.1)', borderRadius:16, padding:18 }}>
            ...
            <div style={{ fontSize:12, color:'rgba(255,255,255,0.5)', lineHeight:1.75 }}>
              The 18 Siddhas confirm: your soul chose Uddevalla as a stillness-incubation zone — far from the chaos of karmic hotspots — to build the vessel before the dharmic mission launches globally. The pull toward India is not nostalgia. It is the Graha-Geographic Intelligence of your Pushya soul recognizing its origin field. Move there in consciousness first through the JyotishChamber, then in body when the Dasha calls.
            </div>
          </div>
        </div>
      )}
