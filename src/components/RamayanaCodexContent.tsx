...
import { RAMAYANA_KANDAS } from '@/data/ramayanaCodexContent';
...
const RamayanaCodexContent: React.FC<{ moduleKey: string; dbModuleId: string }> = ({ moduleKey, dbModuleId }) => {
  const kanda = RAMAYANA_KANDAS.find((k) => k.id === moduleKey);
  const [openIndex, setOpenIndex] = React.useState(0);
  const { rows: sectionRows, toggleSectionComplete, setSectionNotes } = useSectionProgress(dbModuleId, 'user_ramayana_codex_section_progress');
  const [noteSaveStatus, setNoteSaveStatus] = React.useState<Record<string, 'idle' | 'saving' | 'saved' | 'error'>>({});

  if (!kanda) {
    return <p style={bodyStyle}>This Kanda's content is being prepared.</p>;
  }

  type Card = { sectionId: string; title: string; render: () => React.ReactNode };
  const cards: Card[] = [
    { sectionId: 'intro', title: `${kanda.name} — Overview`, render: () => (
      <>
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginBottom: 12 }}>
          {kanda.chakra} · {kanda.element} · Narrated through {kanda.voice}
        </p>
        <p style={bodyStyle}>{kanda.intro}</p>
      </>
    ) },
    ...kanda.secrets.map((s, i): Card => ({
      sectionId: `secret-${i}`, title: s.title, render: () => (
        <>
          <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: '.3em', textTransform: 'uppercase', color: AMBER, marginBottom: 8 }}>Teaching</div>
          <p style={{ ...bodyStyle, marginBottom: 16 }}>{s.teaching}</p>
          <div style={{ background: 'rgba(217,119,6,0.05)', border: '1px solid rgba(217,119,6,0.18)', borderRadius: 18, padding: '18px 20px', marginBottom: 12 }}>
            <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: '.3em', textTransform: 'uppercase', color: AMBER, marginBottom: 10 }}>Siddha Revelation</div>
            <p style={{ fontSize: 17, lineHeight: 1.75, color: 'rgba(255,255,255,0.85)', fontFamily: "'Cormorant Garamond',serif", margin: 0 }}>{s.siddhaRevelation}</p>
          </div>
          <div style={{ background: 'rgba(212,175,55,0.05)', border: '1px solid rgba(212,175,55,0.18)', borderRadius: 18, padding: '18px 20px', marginBottom: 12 }}>
            <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: '.3em', textTransform: 'uppercase', color: '#D4AF37', marginBottom: 10 }}>Activation</div>
            <p style={{ fontSize: 16, lineHeight: 1.75, color: 'rgba(255,255,255,0.8)', fontFamily: "'Cormorant Garamond',serif", margin: 0 }}>{s.activation}</p>
          </div>
          <div style={{ textAlign: 'center', padding: '14px 0' }}>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 19, fontWeight: 600, color: AMBER, margin: '0 0 6px' }}>{s.mantra}</p>
            {s.mantraTranslation && <p style={{ fontSize: 13, fontStyle: 'italic', color: 'rgba(255,255,255,0.5)', margin: 0 }}>{s.mantraTranslation}</p>}
          </div>
        </>
      ),
    })),
          ...
          <div key={c.sectionId} style={{
            ...
            <button type="button" onClick={() => setOpenIndex(isOpen ? -1 : i)} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '1.1rem', fontWeight: 700, color: isOpen ? AMBER : 'rgba(255,255,255,0.85)' }}>
                  {c.title}
                </div>
              </div>
...
export default RamayanaCodexContent;
