...
import { MEDIUMSHIP_CURRICULUM } from '@/data/mediumshipModuleContent';
...
const MediumshipAcademyContent: React.FC<{ moduleKey: string; dbModuleId: string }> = ({ moduleKey, dbModuleId }) => {
  const num = parseInt(moduleKey.replace('m', ''), 10);
  const mModule = MEDIUMSHIP_CURRICULUM[num - 1];
  const [openIndex, setOpenIndex] = React.useState(0);
  const { rows: sectionRows, toggleSectionComplete, setSectionNotes } = useSectionProgress(dbModuleId, 'user_mediumship_academy_section_progress');
            ...
            {isOpen && (
              <div style={{ padding: '0 20px 24px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 18, flexWrap: 'wrap', gap: 10 }}>
                  ...
                  <button type="button" disabled={i === mModule.lessons.length - 1} onClick={() => setOpenIndex(Math.min(mModule.lessons.length - 1, i + 1))} style={{
                    background: i === mModule.lessons.length - 1 ? 'transparent' : 'rgba(167,139,250,0.14)',
                    border: `1px solid ${i === mModule.lessons.length - 1 ? 'rgba(255,255,255,0.1)' : 'rgba(167,139,250,0.4)'}`,
                    color: i === mModule.lessons.length - 1 ? 'rgba(255,255,255,0.25)' : VIOLET,
                    padding: '9px 16px', borderRadius: 999, fontSize: 10.5, fontWeight: 700,
                    cursor: i === mModule.lessons.length - 1 ? 'default' : 'pointer',
                  }}>
                    {i === mModule.lessons.length - 1 ? 'Last lesson' : 'Next lesson →'}
                  </button>
                </div>
              </div>
            )}
...
export default MediumshipAcademyContent;
