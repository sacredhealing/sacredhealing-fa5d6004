...
/* ─── Hormone curve data (28-day reference) ─────────────────────────────────── */
...
function HormoneChart({ currentDay, chartDatasetLabels }: { currentDay: number; chartDatasetLabels: string[] }) {
    ...
    const buildChart = () => {
      ...
      chartRef.current = new window.Chart(ctx, {
        ...
        options: {
          ...
          scales: {
            x: { grid: { color: 'rgba(255,255,255,0.04)' }, ticks: { color: 'rgba(255,255,255,0.3)', font: { size: 9 }, maxTicksLimit: 8 }, border: { color: 'transparent' } },
  ...
  useEffect(() => {
    ...
    ds.forEach((d: any, i: number) => {
      const radii = Array(28).fill(0); radii[currentDay - 1] = 5;
      d.pointRadius = radii; d.pointBackgroundColor = colors[i]; d.pointBorderColor = '#050505'; d.pointBorderWidth = 2;
    });
...
Analyse this food photo and provide a detailed breakdown:

1. **Identified Foods** – list each food visible in the image
2. **Estimated Nutrients per serving** – include:
   - Protein (g)
   - Carbohydrates (g)
   - Fats (g)
   - Key minerals: Iron, Zinc, Magnesium, Calcium, Potassium, Boron (mg where possible)
   - Key vitamins: B6, B9 (Folate), B12, C, D, E, K2
3. **Hormonal Phase Alignment** – rate each food: ✅ excellent for this phase | ⚠️ neutral | ❌ avoid
4. **WomanCode Verdict** – one sentence on how this meal supports or challenges the user's hormones today
5. **Vedic + Ayurvedic Note** – which dosha does this meal pacify or aggravate? Any Sattvic upgrades?

Keep the response structured, clear, and empowering. All recommendations must be plant-based (Ghee and raw Honey are permitted as Vedic exceptions).`;

      const { data, error: fnErr } = await supabase.functions.invoke('gemini-bridge', {
        body: {
          prompt,
      ...
      if (fnErr || !data?.text) throw new Error(fnErr?.message || 'No response');
      setResult(data.text);
    } catch (e: any) {
      setError(e.message || ui.photoTab.errorBody);
    } finally {
      setLoading(false);
    }
  ...
  const btnStyle = (primary = false) => ({
    ...
    cursor: 'pointer',
    fontFamily: 'inherit',
    border: primary ? 'none' : `1px solid rgba(212,175,55,0.4)`,
    background: primary ? gold : 'rgba(212,175,55,0.1)',
    color: primary ? '#050505' : gold,
    transition: 'all 0.2s',
  });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ background: 'linear-gradient(135deg, rgba(212,175,55,0.08), rgba(212,175,55,0.02))', border: '1px solid rgba(212,175,55,0.2)', borderRadius: 28, padding: 24 }}>
        ...
        {imageUrl && (
          <div style={{ marginBottom: 16, borderRadius: 18, overflow: 'hidden', border: '1px solid rgba(212,175,55,0.2)' }}>
            <img src={imageUrl} alt="meal" style={{ width: '100%', maxHeight: 260, objectFit: 'cover', display: 'block' }} />
          </div>
        )}
        ...
        {phase && (
          <div style={{ marginTop: 12, fontSize: '9px', fontWeight: 700, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.2em', textTransform: 'uppercase' }}>
            {interpolate(pt.phaseContext, { phase: phase.name, day })}
          </div>
        )}
      </div>

      {loading && (
        <div style={{ textAlign: 'center', padding: '28px 20px', color: 'rgba(255,255,255,0.5)', fontSize: 12 }}>
      ...
      {error && (
        <div style={{ background: 'rgba(255,80,80,0.08)', border: '1px solid rgba(255,80,80,0.2)', borderRadius: 20, padding: 18 }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: '#FC8181', marginBottom: 4 }}>{pt.errorTitle}</div>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}>{error}</div>
        </div>
      )}

      {result && (
        <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(212,175,55,0.2)', borderRadius: 24, padding: 22 }}>
          <div style={{ fontSize: '7px', fontWeight: 800, letterSpacing: '0.4em', textTransform: 'uppercase', color: gold, marginBottom: 14 }}>AI Analysis Result</div>
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.78)', lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>{result}</div>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ ...s.glass, borderColor: `${stage.color}22` }}>
        ...
        <div style={{ marginTop: 12, background: 'rgba(212,175,55,0.06)', border: '1px solid rgba(212,175,55,0.15)', borderRadius: 16, padding: 14 }}>
          <div style={{ fontSize: '8px', fontWeight: 800, letterSpacing: '0.3em', textTransform: 'uppercase', color: s.gold, marginBottom: 8 }}>✦ Rasayana (Herbal Protocol)</div>
...
export default function WomanCodeDashboard() {
  ...
  const chartDatasetLabels = useMemo(
    () => HORMONE_ORDER.map((k) => bundle.chartLabels?.[k] ?? k),
    [bundle.chartLabels]
  );
  ...
  const openVedic = (v: { icon: string; t: string; s: string }) =>
    openModal(
      <div>
        ...
        <div style={{ fontSize: '8px', fontWeight: 800, letterSpacing: '0.4em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', marginBottom: 16 }}>
        ...
        {mode === 'cycle' && (
          <>
            ...
            <div style={s.glass}>
              ...
              <input type="range" min={1} max={28} value={day} aria-label={bundle.ui?.sliderLabel} onChange={(e) => setDay(+e.target.value)}
                style={{ width: '100%', accentColor: s.gold, cursor: 'pointer', height: 4 }} />
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
                {pipLabels.map((l, i) => (
                  <button key={i} type="button" onClick={() => setDay(i === 0 ? 3 : i === 1 ? 9 : i === 2 ? 14 : 22)}
