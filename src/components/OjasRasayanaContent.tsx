...
import { OJAS_CURRICULUM } from '@/data/ojasRasayanaModuleContent';
...
const ALL_MODULES = OJAS_CURRICULUM.flatMap((t) => t.modules);

const OjasRasayanaContent: React.FC<{ moduleKey: string; dbModuleId: string }> = ({ moduleKey, dbModuleId }) => {
  const oModule = ALL_MODULES.find((m) => `m${m.num}` === moduleKey);
  const [openIndex, setOpenIndex] = React.useState(0);
  const { rows: sectionRows, toggleSectionComplete, setSectionNotes } = useSectionProgress(dbModuleId, 'user_ojas_rasayana_section_progress');
            ...
            {isOpen && (
              <div style={{ padding: '0 20px 24px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 18, flexWrap: 'wrap', gap: 10 }}>
                  ...
                  <button type="button" disabled={i === oModule.lessons.length - 1} onClick={() => setOpenIndex(Math.min(oModule.lessons.length - 1, i + 1))} style={{
                    background: i === oModule.lessons.length - 1 ? 'transparent' : 'rgba(20,184,166,0.14)',
                    border: `1px solid ${i === oModule.lessons.length - 1 ? 'rgba(255,255,255,0.1)' : 'rgba(20,184,166,0.4)'}`,
                    color: i === oModule.lessons.length - 1 ? 'rgba(255,255,255,0.25)' : TEAL,
                    padding: '9px 16px', borderRadius: 999, fontSize: 10.5, fontWeight: 700,
                    cursor: i === oModule.lessons.length - 1 ? 'default' : 'pointer',
                  }}>
                    {i === oModule.lessons.length - 1 ? 'Last lesson' : 'Next lesson →'}
                  </button>
                </div>
              </div>
            )}
...
export default OjasRasayanaContent;
