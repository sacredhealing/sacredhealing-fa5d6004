...
export default function UserManagementPanel() {
  ...
  const handleGrantTier = async () => {
    ...
    try {
      await supabase.from("admin_granted_access").update({ is_active: false }).eq("user_id", selectedUser.id).eq("access_type", "membership");
      if (newTier !== "free") {
        const { error } = await supabase.from("admin_granted_access").insert({
          user_id: selectedUser.id, access_type: "membership",
          tier: newTier, access_id: newTier,
          is_active: true, granted_at: new Date().toISOString(),
        });
        if (error) throw error;
      }
      setUsers(prev => prev.map(u => u.id === selectedUser.id ? { ...u, tier: newTier } : u));
      toast({ title: "Tier Updated", description: `${selectedUser.full_name || "User"} set to ${TIER_LABELS[newTier] || newTier}` });
      setModalMode(null);
    } catch (e: any) {
  ...
  const handleDelete = async (userId: string) => {
    setActionLoading(true);
    // Optimistic removal first so UI feels instant
    ...
    try {
      ...
      toast({ title: "Profile Removed", description: "Profile cleared. Auth account: delete in Supabase dashboard." });
    } catch (e: any) {
    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans', Inter, sans-serif", color: "#fff", paddingBottom: 60 }}>
      ...
      {loading ? (
        ...
        <div style={{ ...glass, overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1.4fr 1fr auto", gap: 16, padding: "12px 20px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          ...
          {filtered.length === 0 ? (
            ...
            return (
              <div key={user.id} style={{ display: "grid", gridTemplateColumns: "2fr 1.4fr 1fr auto", gap: 16, padding: "13px 20px", borderBottom: "1px solid rgba(255,255,255,0.03)", alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: user.full_name ? "#fff" : "#f59e0b" }}>
          ...
          {modalMode === "view" ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                ...
                ["Tier", TIER_LABELS[selectedUser.tier] || "Free"],
                ...
                ["Birth Place", selectedUser.birth_place || "Not set"],
      ...
      {confirmDelete && (
        <SQIModal onClose={() => setConfirmDelete(null)}>
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.5em", textTransform: "uppercase", color: "#ef4444", marginBottom: 8 }}>IRREVERSIBLE</div>
            <h3 style={{ fontSize: 20, fontWeight: 900, color: "#fff", margin: "0 0 12px" }}>Delete this profile?</h3>
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 13, lineHeight: 1.6, marginBottom: 20 }}>
              Removes profile data and access grants instantly. To also remove the auth/login account, use the Supabase dashboard.
            </p>
            <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
              <SQIBtn label="Cancel" onClick={() => setConfirmDelete(null)} />
              <SQIBtn label={actionLoading ? "Deleting..." : "Yes, Delete"} onClick={() => handleDelete(confirmDelete)} color="#ef4444" loading={actionLoading} />
            </div>
          </div>
        </SQIModal>
      )}
