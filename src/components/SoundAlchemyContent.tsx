...
import { SOUND_ALCHEMY_MODULES } from '@/data/soundAlchemyModuleContent';
...
const SoundAlchemyContent: React.FC<{ moduleKey: string; dbModuleId: string }> = ({ moduleKey, dbModuleId }) => {
  const num = parseInt(moduleKey.replace('m', ''), 10);
  const sModule = SOUND_ALCHEMY_MODULES.find((m) => m.num === num);
  const [openIndex, setOpenIndex] = React.useState(0);
  const { rows: sectionRows, toggleSectionComplete, setSectionNotes } = useSectionProgress(dbModuleId, 'user_sound_alchemy_section_progress');
          ...
          <div key={sectionId} style={{
            ...
            {isOpen && (
              <div style={{ padding: '0 20px 24px' }}>
                <p style={bodyStyle}>{lesson.body}</p>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 18, flexWrap: 'wrap', gap: 10 }}>
                  ...
                  <button type="button" disabled={i === sModule.lessons.length - 1} onClick={() => setOpenIndex(Math.min(sModule.lessons.length - 1, i + 1))} style={{
                    background: i === sModule.lessons.length - 1 ? 'transparent' : 'rgba(217,119,6,0.14)',
                    border: `1px solid ${i === sModule.lessons.length - 1 ? 'rgba(255,255,255,0.1)' : 'rgba(217,119,6,0.4)'}`,
                    color: i === sModule.lessons.length - 1 ? 'rgba(255,255,255,0.25)' : AMBER,
                    padding: '9px 16px', borderRadius: 999, fontSize: 10.5, fontWeight: 700,
                    cursor: i === sModule.lessons.length - 1 ? 'default' : 'pointer',
                  }}>
                    {i === sModule.lessons.length - 1 ? 'Last lesson' : 'Next lesson →'}
                  </button>
                </div>
              </div>
            )}
...
export default SoundAlchemyContent;
