...
import { HOLY_SCIENCE_MODULES } from '@/data/holyScienceModuleContent';
...
const HolyScienceModuleContent: React.FC<{ moduleKey: string; dbModuleId: string }> = ({ moduleKey, dbModuleId }) => {
  const hModule = HOLY_SCIENCE_MODULES.find((m) => `m${m.id}` === moduleKey);
  const [openIndex, setOpenIndex] = React.useState(0);
  const { rows: sectionRows, toggleSectionComplete, setSectionNotes } = useSectionProgress(dbModuleId, 'user_holy_science_section_progress');
          ...
          <div key={sectionId} style={{
            ...
            {isOpen && (
              <div style={{ padding: '0 20px 24px' }}>
                <p style={{ ...bodyStyle, marginBottom: lesson.revelation ? 18 : 0 }}>{renderBody(lesson.content)}</p>

                {lesson.revelation && (
                  <div style={{ background: 'rgba(212,175,55,0.05)', border: '1px solid rgba(212,175,55,0.18)', borderRadius: 18, padding: '18px 20px' }}>
                    <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: '.3em', textTransform: 'uppercase', color: '#D4AF37', marginBottom: 10 }}>Akasha-Archive Transmission</div>
                    <p style={{ fontSize: 17, lineHeight: 1.75, color: 'rgba(255,255,255,0.85)', fontFamily: "'Cormorant Garamond',serif", margin: 0, fontStyle: 'italic' }}>{lesson.revelation}</p>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 18, flexWrap: 'wrap', gap: 10 }}>
                  ...
                  <button type="button" disabled={i === hModule.lessons.length - 1} onClick={() => setOpenIndex(Math.min(hModule.lessons.length - 1, i + 1))} style={{
                    background: i === hModule.lessons.length - 1 ? 'transparent' : 'rgba(167,139,250,0.14)',
                    border: `1px solid ${i === hModule.lessons.length - 1 ? 'rgba(255,255,255,0.1)' : 'rgba(167,139,250,0.4)'}`,
                    color: i === hModule.lessons.length - 1 ? 'rgba(255,255,255,0.25)' : VIOLET,
                    padding: '9px 16px', borderRadius: 999, fontSize: 10.5, fontWeight: 700,
                    cursor: i === hModule.lessons.length - 1 ? 'default' : 'pointer',
                  }}>
                    {i === hModule.lessons.length - 1 ? 'Last lesson' : 'Next lesson →'}
                  </button>
                </div>
              </div>
            )}
...
export default HolyScienceModuleContent;
