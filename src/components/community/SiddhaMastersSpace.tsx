...
export default function SiddhaMastersSpace({ isAdmin, onBack, userTier }: Props) {
  ...
  const loadEntries = useCallback(async () => {
    ...
    const { data, error } = await (supabase as any)
      .from("siddha_masters_entries")
      .select("*")
      .order("created_at", { ascending: false });
    if (error) {
      console.error("Failed to load Siddha Masters & Saints entries:", error);
      setEntries([]);
    } else {
      setEntries(data || []);
    }
    ...
    try {
      const { data, error } = await supabase.functions.invoke("gemini-bridge", {
        body: {
          ...
          feature: "siddha_masters_translation",
        },
      });
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const { error } = await (supabase as any).from("siddha_masters_entries").insert({
      ...
      if (error) throw error;
      toast.success("Posted to Siddha Masters & Saints");
      setForm({ ...emptyForm });
      setShowForm(false);
      loadEntries();
    } catch (e: any) {
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    const { error } = await (supabase as any).from("siddha_masters_entries").delete().eq("id", id);
    if (error) {
    ...
    <div className="c-chat-view">
      <div className="c-chat-header">
        ...
        <div className="c-chat-title">
          <div className="c-chat-name">Siddha Masters & Saints</div>
          <div className="c-chat-sub">Wisdom from the lineage of Siddha masters and saints</div>
        </div>
      </div>
        ...
        {showForm && isAdmin && (
          <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid rgba(212,175,55,.25)", borderRadius: 16, padding: 16, marginBottom: 20, display: "flex", flexDirection: "column", gap: 10 }}>
            <div>
              ...
              <input
                list="siddha-masters-categories"
                placeholder="e.g. Babaji, Yogananda, Lahiri Mahasaya…"
                value={form.category}
                onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))}
                style={inputStyle}
              />
              <datalist id="siddha-masters-categories">
            ...
            <div>
              ...
              <input
                list="siddha-masters-transmitters"
                value={form.transmitter}
                onChange={(e) => setForm((f) => ({ ...f, transmitter: e.target.value }))}
                style={inputStyle}
              />
              <datalist id="siddha-masters-transmitters">
            ...
            >
              {saving ? "Posting…" : "Post to Siddha Masters & Saints"}
            </button>
          </div>
        )}
