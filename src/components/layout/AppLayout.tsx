...
import { ActiveSiteBadge } from '@/components/resonance/GlobalResonanceHub';

const pageVariants = {
  initial: { opacity: 1, y: 0, filter: 'blur(0px)' },
  animate: { opacity: 1, y: 0, filter: 'blur(0px)' },
  exit: { opacity: 0.7, y: -4, filter: 'blur(2px)' },
};
    ...
    <div className="min-h-screen relative">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none -z-10">
        <motion.div
          ...
          style={{ background: 'radial-gradient(circle, hsl(271 76% 53% / 0.15) 0%, transparent 70%)', filter: 'blur(60px)' }}
          animate={{ x: [0, 30, 0], y: [0, 20, 0], scale: [1, 1.1, 1] }}
          transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut' }}
        />
        <motion.div
          className="absolute bottom-1/4 right-0 w-[400px] h-[400px] rounded-full"
          style={{ background: 'radial-gradient(circle, hsl(174 72% 56% / 0.12) 0%, transparent 70%)', filter: 'blur(50px)' }}
          animate={{ x: [0, -25, 0], y: [0, -15, 0], scale: [1, 1.15, 1] }}
          transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut', delay: 2 }}
        />
        <motion.div
          className="absolute top-1/2 left-0 w-[350px] h-[350px] rounded-full"
          style={{ background: 'radial-gradient(circle, hsl(51 100% 50% / 0.08) 0%, transparent 70%)', filter: 'blur(45px)' }}
