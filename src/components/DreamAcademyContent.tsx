...
import { DREAM_CURRICULUM } from '@/data/dreamAcademyModuleContent';
...
const DreamAcademyContent: React.FC<{ moduleKey: string; dbModuleId: string }> = ({ moduleKey, dbModuleId }) => {
  const num = parseInt(moduleKey.replace('m', ''), 10);
  const dModule = DREAM_CURRICULUM[num - 1];
  const [openIndex, setOpenIndex] = React.useState(0);
  const { rows: sectionRows, toggleSectionComplete, setSectionNotes } = useSectionProgress(dbModuleId, 'user_dream_academy_section_progress');
            ...
            {isOpen && (
              <div style={{ padding: '0 20px 24px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 18, flexWrap: 'wrap', gap: 10 }}>
                  ...
                  <button type="button" disabled={i === dModule.lessons.length - 1} onClick={() => setOpenIndex(Math.min(dModule.lessons.length - 1, i + 1))} style={{
                    background: i === dModule.lessons.length - 1 ? 'transparent' : 'rgba(167,139,250,0.14)',
                    border: `1px solid ${i === dModule.lessons.length - 1 ? 'rgba(255,255,255,0.1)' : 'rgba(167,139,250,0.4)'}`,
                    color: i === dModule.lessons.length - 1 ? 'rgba(255,255,255,0.25)' : VIOLET,
                    padding: '9px 16px', borderRadius: 999, fontSize: 10.5, fontWeight: 700,
                    cursor: i === dModule.lessons.length - 1 ? 'default' : 'pointer',
                  }}>
                    {i === dModule.lessons.length - 1 ? 'Last lesson' : 'Next lesson →'}
                  </button>
                </div>
              </div>
            )}
...
export default DreamAcademyContent;
