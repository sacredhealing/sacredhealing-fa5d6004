...
export const ChangePasswordDialog: React.FC<ChangePasswordDialogProps> = ({ open, onOpenChange }) => {
  ...
  const handleChange = async (e: React.FormEvent) => {
    e.preventDefault();
    if (newPw.length < 8) {
      toast({ title: t('changePassword.tooShort'), description: t('changePassword.min8'), variant: 'destructive' }); return;
    }
    if (newPw !== confirmPw) {
      toast({ title: t('changePassword.noMatch'), description: t('changePassword.noMatchDesc'), variant: 'destructive' }); return;
    }
    setChanging(true);
    const { error } = await supabase.auth.updateUser({ password: newPw });
    setChanging(false);
    if (error) {
      toast({ title: t('common.error.generic'), description: error.message, variant: 'destructive' });
    } else {
      setChanged(true);
  ...
  const handleReset = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!resetEmail) {
      toast({ title: t('changePassword.enterEmail'), description: t('changePassword.emailEmpty'), variant: 'destructive' }); return;
    }
    setSending(true);
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, backdropFilter: 'blur(16px)', background: 'rgba(0,0,0,.55)' }}
      ...
      <div style={{ width: '100%', maxWidth: 420, background: '#0a0a0a', border: '1px solid rgba(212,175,55,.18)', borderRadius: 32, padding: '36px 28px', position: 'relative', boxShadow: '0 0 60px rgba(212,175,55,.1), 0 24px 60px rgba(0,0,0,.7)' }}
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h2 style={{ fontSize: 22, fontWeight: 900, letterSpacing: '-.03em', color: G, margin: 0 }}>{t('changePassword.title')}</h2>
        ...
        <div style={{ display: 'flex', gap: 4, background: 'rgba(255,255,255,.04)', borderRadius: 100, padding: 4, marginBottom: 28 }}>
          {(['change', 'reset'] as const).map(tabKey => (
            <button key={tabKey} type="button" onClick={() => setTab(tabKey)}
              style={{ flex: 1, padding: '10px', borderRadius: 100, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 800, letterSpacing: '.25em', textTransform: 'uppercase', fontFamily: "'Montserrat', sans-serif", transition: 'all .2s',
                background: tab === tabKey ? G : 'transparent',
                color: tab === tabKey ? '#050505' : 'rgba(255,255,255,.4)',
                boxShadow: tab === tabKey ? '0 0 20px rgba(212,175,55,.3)' : 'none',
              }}>
              {tabKey === 'change' ? t('changePassword.tabChange') : t('changePassword.tabForgot')}
            </button>
          ))}
        </div>
        ...
        {tab === 'change' && (
          changed ? (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <p style={{ fontWeight: 900, fontSize: 18, color: '#4ade80' }}>{t('changePassword.updated')}</p>
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', marginTop: 6 }}>{t('changePassword.updatedDesc')}</p>
            </div>
          ) : (
            <form onSubmit={handleChange} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ position: 'relative' }}>
                <label style={{ fontSize: 8, fontWeight: 800, letterSpacing: '.4em', textTransform: 'uppercase', color: 'rgba(255,255,255,.3)', display: 'block', marginBottom: 8, fontFamily: "'Montserrat', sans-serif" }}>{t('changePassword.newPassword')}</label>
                <input type={showNew ? 'text' : 'password'} value={newPw} onChange={e => setNewPw(e.target.value)}
                  style={inputStyle} placeholder={t('changePassword.min8Placeholder')} autoComplete="new-password"
              ...
              <div style={{ position: 'relative' }}>
                <label style={{ fontSize: 8, fontWeight: 800, letterSpacing: '.4em', textTransform: 'uppercase', color: 'rgba(255,255,255,.3)', display: 'block', marginBottom: 8, fontFamily: "'Montserrat', sans-serif" }}>{t('changePassword.confirmPassword')}</label>
                <input type={showConfirm ? 'text' : 'password'} value={confirmPw} onChange={e => setConfirmPw(e.target.value)}
                  ...
                  placeholder={t('changePassword.repeatPassword')} autoComplete="new-password"
              ...
              <button type="submit" disabled={changing}
                ...
                {changing ? <><Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> {t('changePassword.updating')}</> : `⚡ ${t('changePassword.updateBtn')}`}
              </button>
            </form>
          )
        )}
        ...
        {tab === 'reset' && (
          sent ? (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              <CheckCircle size={48} color="#4ade80" style={{ margin: '0 auto 14px' }} />
              <p style={{ fontWeight: 900, fontSize: 18, color: '#4ade80' }}>{t('changePassword.resetSent')}</p>
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,.45)', marginTop: 6, lineHeight: 1.7 }}>
                {t('changePassword.resetSentDesc')} <strong style={{ color: G }}>{resetEmail}</strong> {t('changePassword.resetSentLink')}
              </p>
            </div>
          ) : (
            <form onSubmit={handleReset} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,.45)', lineHeight: 1.7, margin: 0 }}>
                {t('changePassword.resetDesc')}
              </p>
              <div style={{ position: 'relative' }}>
                <label style={{ fontSize: 8, fontWeight: 800, letterSpacing: '.4em', textTransform: 'uppercase', color: 'rgba(255,255,255,.3)', display: 'block', marginBottom: 8, fontFamily: "'Montserrat', sans-serif" }}>{t('changePassword.emailLabel')}</label>
              ...
              <button type="submit" disabled={sending}
                ...
                {sending ? <><Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> {t('changePassword.sending')}</> : `✉ ${t('changePassword.sendResetBtn')}`}
              </button>
            </form>
          )
        )}
