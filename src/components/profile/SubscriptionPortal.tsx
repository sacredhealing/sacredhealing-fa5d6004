...
function DaysBar({ days, t }: { days: number; t: (k: string, o?: any) => string }) {
  const pct = Math.min(100, Math.round((days / 30) * 100));
  const color = days < 5 ? "#ef4444" : days < 10 ? "#f59e0b" : "#22c55e";
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
        <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 8, fontWeight: 800, letterSpacing: "0.4em", textTransform: "uppercase" }}>{t("subscriptionPortal.labelDaysRemaining")}</span>
        <span style={{ color, fontSize: 13, fontWeight: 800 }}>{days} {t("subscriptionPortal.labelDays")}</span>
      </div>
...
export function SubscriptionPortal({ isOpen, onClose }: Props) {
  ...
  async function openStripePortal() {
    ...
    try {
      ...
      const res = await supabase.functions.invoke("stripe-billing-portal", {
        headers: { Authorization: `Bearer ${session.access_token}` },
      });
      if (res.data?.url) {
        ...
        setActiveTab("change");
        setMsg({ text: t("subscriptionPortal.noBillingPortal"), ok: true });
      }
    } catch (e: any) {
      setMsg({ text: t("subscriptionPortal.portalError"), ok: false });
  ...
  async function handleReactivate() {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) throw new Error("No session");
      const res = await supabase.functions.invoke("stripe-reactivate-subscription", {
        headers: { Authorization: `Bearer ${session.access_token}` },
      });
      if (res.data?.success) {
        await refreshMembership?.();
  ...
  // ─── Tier config built from translations ──
  const TIER_CONFIG: Record<string, { label: string; glyph: string; color: string; glow: string; price: string; features: string[] }> = {
    free:              { label: t("subscriptionPortal.tierFreeSeeker"), glyph: "◯", color: "rgba(255,255,255,0.4)", glow: "rgba(255,255,255,0.08)", price: t("subscriptionPortal.priceFree"), features: [t("subscriptionPortal.tierFreeFeature1"), t("subscriptionPortal.tierFreeFeature2"), t("subscriptionPortal.tierFreeFeature3")] },
    "prana-flow":      { label: "Prana-Flow",      glyph: "◈", color: "#22D3EE", glow: "rgba(34,211,238,0.2)",   price: "€19/mo", features: [t("subscriptionPortal.tierPranaFeature1"), t("subscriptionPortal.tierPranaFeature2"), t("subscriptionPortal.tierPranaFeature3"), t("subscriptionPortal.tierPranaFeature4")] },
    "siddha-quantum":  { label: "Siddha-Quantum",  glyph: "✦", color: "#D4AF37", glow: "rgba(212,175,55,0.25)", price: "€45/mo", features: [t("subscriptionPortal.tierSiddhaFeature1"), t("subscriptionPortal.tierSiddhaFeature2"), t("subscriptionPortal.tierSiddhaFeature3"), t("subscriptionPortal.tierSiddhaFeature4")] },
    "akasha-infinity": { label: "Akasha-Infinity", glyph: "∞", color: "#D4AF37", glow: "rgba(212,175,55,0.4)",  price: t("subscriptionPortal.priceAkasha"), features: [t("subscriptionPortal.tierAkashaFeature1"), t("subscriptionPortal.tierAkashaFeature2"), t("subscriptionPortal.tierAkashaFeature3"), t("subscriptionPortal.tierAkashaFeature4")] },
  };
  ...
  const upgrades = TIER_ORDER.slice(currentIdx + 1).map(k => ({ key: k, ...TIER_CONFIG[k] }));
  const downgrades = TIER_ORDER.slice(1, currentIdx).map(k => ({ key: k, ...TIER_CONFIG[k] }));
  ...
  const tabBtn = (tabKey: string): React.CSSProperties => ({
    flex: 1, padding: "9px 0", borderRadius: 14, border: "none", cursor: "pointer",
    fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase",
    background: activeTab === tabKey ? "rgba(212,175,55,0.15)" : "transparent",
    color: activeTab === tabKey ? "#D4AF37" : "rgba(255,255,255,0.35)",
  });

  const statusText = isLifetime ? t("subscriptionPortal.statusEternal")
    : adminGranted ? t("subscriptionPortal.statusAdminGranted")
    : cancelAtEnd ? t("subscriptionPortal.statusCancelling")
    : isPaid ? t("subscriptionPortal.statusActive")
    : t("subscriptionPortal.statusFree");
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "min(94vw,640px)", maxHeight: "92vh", overflowY: "auto", background: "rgba(6,6,6,0.98)", border: "1px solid rgba(212,175,55,0.2)", borderRadius: 40, padding: "36px 36px", zIndex: 9001, boxShadow: "0 0 100px rgba(212,175,55,0.07), 0 40px 120px rgba(0,0,0,0.9)" }}>
        ...
        {adminGranted && (
          <div style={{ marginBottom: 16, padding: "10px 16px", borderRadius: 14, background: "rgba(167,139,250,0.08)", border: "1px solid rgba(167,139,250,0.25)", color: "#a78bfa", fontSize: 12, fontWeight: 700 }}>
            {t("subscriptionPortal.adminGrantedBadge")} {tc.label}
          </div>
        )}

        {/* Tabs */}
        <div style={{ display: "flex", gap: 6, marginBottom: 24, background: "rgba(255,255,255,0.03)", borderRadius: 18, padding: 4 }}>
          <button onClick={() => setActiveTab("overview")} style={tabBtn("overview")}>{t("subscriptionPortal.tabPlan")}</button>
          <button onClick={() => setActiveTab("change")}   style={tabBtn("change")}>{t("subscriptionPortal.tabChange")}</button>
          <button onClick={() => setActiveTab("receipts")} style={tabBtn("receipts")}>{t("subscriptionPortal.tabReceipts")}</button>
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${tc.glow}`, borderRadius: 26, padding: 26, boxShadow: `0 0 40px ${tc.glow}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 18 }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={lbl}>{t("subscriptionPortal.labelCurrentPlan")}</p>
                  <p style={{ color: tc.color, fontSize: 20, fontWeight: 900, letterSpacing: "-0.04em", margin: 0 }}>{tc.label}</p>
                </div>
                <div style={{ textAlign: "right" }}>
                  <p style={lbl}>{t("subscriptionPortal.labelRate")}</p>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {memberSince && (
                <div style={card}>
                  <p style={lbl}>{t("subscriptionPortal.labelMemberSince")}</p>
                  <p style={{ color: "#fff", fontSize: 13, fontWeight: 700, margin: 0 }}>{fmt(memberSince)}</p>
                </div>
              )}
              <div style={card}>
                <p style={lbl}>{t("subscriptionPortal.labelStatus")}</p>
                <div style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 2 }}>
                  ...
                  <span style={{ color: "#fff", fontSize: 13, fontWeight: 700 }}>{statusText}</span>
                </div>
              </div>

              {!isLifetime && !adminGranted && daysLeft !== null && (
                <div style={card}>
                  <DaysBar days={daysLeft} t={t} />
                  {periodEnd && (
                    <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 11, marginTop: 8, marginBottom: 0 }}>
                      {cancelAtEnd ? t("subscriptionPortal.accessEndsPrefix") : t("subscriptionPortal.renewsPrefix")} {fmt(periodEnd)}
                    </p>
                  )}
                </div>
              )}
              ...
              {isLifetime && (
                <div style={{ ...card, border: "1px solid rgba(212,175,55,0.25)", textAlign: "center", padding: 20 }}>
                  ...
                  <p style={{ color: "#D4AF37", fontSize: 14, fontWeight: 700, margin: 0 }}>{t("subscriptionPortal.lifetimeBadgeTitle")}</p>
                  <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, marginTop: 4 }}>{t("subscriptionPortal.lifetimeBadgeDesc")}</p>
                </div>
              )}

              {stripeLoading && !stripeData && !adminGranted && isPaid && (
                <div style={card}>
                  <p style={{ color: "rgba(255,255,255,0.25)", fontSize: 12 }}>{t("subscriptionPortal.loadingBilling")}</p>
                </div>
              )}
            </div>
            ...
            {isPaid && !adminGranted && (
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => setActiveTab("change")} style={{ flex: 1, padding: "12px 0", background: "linear-gradient(135deg,rgba(212,175,55,0.15),rgba(212,175,55,0.05))", border: "1px solid rgba(212,175,55,0.3)", borderRadius: 16, color: "#D4AF37", fontSize: 11, fontWeight: 800, letterSpacing: "0.07em", textTransform: "uppercase", cursor: "pointer" }}>
                  {t("subscriptionPortal.btnChangePlan")}
                </button>
                <button onClick={openStripePortal} disabled={portalLoading} style={{ flex: 1, padding: "12px 0", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, color: "rgba(255,255,255,0.45)", fontSize: 11, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", cursor: "pointer" }}>
                  {portalLoading ? "…" : t("subscriptionPortal.btnPayment")}
                </button>
              </div>
            )}
            ...
            {cancelAtEnd && !adminGranted && (
              <button onClick={handleReactivate} disabled={cancelLoading} style={{ width: "100%", padding: "12px 0", background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: 16, color: "#22c55e", fontSize: 12, fontWeight: 800, letterSpacing: "0.07em", textTransform: "uppercase", cursor: "pointer" }}>
                {cancelLoading ? t("subscriptionPortal.reactivating") : t("subscriptionPortal.reactivateBtn")}
              </button>
            )}
            ...
            {isPaid && !isLifetime && !cancelAtEnd && !adminGranted && (
              <div style={{ background: "rgba(239,68,68,0.03)", border: "1px solid rgba(239,68,68,0.1)", borderRadius: 18, padding: 18 }}>
                <p style={{ ...lbl, color: "rgba(239,68,68,0.4)" }}>{t("subscriptionPortal.labelCancel")}</p>
                {!cancelConfirm ? (
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div>
                      <p style={{ color: "rgba(255,255,255,0.65)", fontSize: 13, fontWeight: 600, margin: 0 }}>{t("subscriptionPortal.cancelTitle")}</p>
                      <p style={{ color: "rgba(255,255,255,0.3)", fontSize: 11, marginTop: 3 }}>{t("subscriptionPortal.cancelDesc")}</p>
                    </div>
                    <button onClick={() => setCancelConfirm(true)} style={{ padding: "8px 18px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 10, color: "#ef4444", fontSize: 11, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>
                      {t("subscriptionPortal.cancelBtn")}
                    </button>
                  </div>
                ) : (
                  <div>
                    <p style={{ color: "#fff", fontSize: 13, fontWeight: 600, marginBottom: 6 }}>{t("subscriptionPortal.cancelConfirmTitle")}</p>
                    <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, marginBottom: 14, lineHeight: 1.5 }}>
                      {t("subscriptionPortal.cancelConfirmDesc")} {periodEnd ? fmt(periodEnd) : t("subscriptionPortal.cancelConfirmDescFallback")}.
                    </p>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={handleCancel} disabled={cancelLoading} style={{ flex: 1, padding: "11px 0", background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.35)", borderRadius: 12, color: "#ef4444", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                        {cancelLoading ? t("subscriptionPortal.cancelling") : t("subscriptionPortal.cancelConfirmYes")}
                      </button>
            ...
            {!isPaid && (
              <div style={{ background: "rgba(212,175,55,0.04)", border: "1px dashed rgba(212,175,55,0.2)", borderRadius: 22, padding: 22, textAlign: "center" }}>
                <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 14, lineHeight: 1.6, marginBottom: 14 }}>
                  {t("subscriptionPortal.freeTierDesc")}
                </p>
                <button onClick={() => setActiveTab("change")} style={{ padding: "12px 28px", background: "linear-gradient(135deg,rgba(212,175,55,0.18),rgba(212,175,55,0.07))", border: "1px solid rgba(212,175,55,0.35)", borderRadius: 14, color: "#D4AF37", fontWeight: 800, fontSize: 13, cursor: "pointer" }}>
                  {t("subscriptionPortal.freeTierCta")}
                </button>
              </div>
            )}
          </div>
        ...
        {activeTab === "change" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 13, lineHeight: 1.6 }}>
              {t("subscriptionPortal.changePlanDesc")}
            </p>
            {upgrades.length > 0 && (
              <>
                <p style={{ ...lbl, color: "rgba(34,197,94,0.7)" }}>{t("subscriptionPortal.upgradeLabel")}</p>
                {upgrades.map(tier => (
                  <PlanCard key={tier.key} config={tier} isUpgrade currentTier={tier.key} onOpenPortal={openStripePortal} t={t} />
                ))}
              </>
            )}
            {downgrades.length > 0 && (
              <>
                <p style={{ ...lbl, marginTop: 6 }}>{t("subscriptionPortal.downgradeLabel")}</p>
                {downgrades.map(tier => (
                  <PlanCard key={tier.key} config={tier} isUpgrade={false} currentTier={tier.key} onOpenPortal={openStripePortal} t={t} />
                ))}
              </>
            )}
            {upgrades.length === 0 && downgrades.length === 0 && (
              <div style={{ textAlign: "center", padding: "40px 0" }}>
                ...
                <p style={{ color: "#D4AF37", fontSize: 17, fontWeight: 700, marginBottom: 6 }}>{t("subscriptionPortal.apexTitle")}</p>
                <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 13 }}>{t("subscriptionPortal.apexDesc")}</p>
              </div>
            )}
          </div>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {stripeLoading && (
              <div style={{ textAlign: "center", padding: "30px 0" }}>
                ...
                <p style={{ color: "rgba(255,255,255,0.3)", fontSize: 12 }}>{t("subscriptionPortal.loadingReceipts")}</p>
              </div>
            )}
            {stripeData?.invoices && stripeData.invoices.length > 0 ? (
              <>
                <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, marginBottom: 4 }}>{t("subscriptionPortal.receiptCount", { count: stripeData.invoices.length })}</p>
              ...
              <div style={{ textAlign: "center", padding: "36px 0" }}>
                ...
                <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 14, marginBottom: 20 }}>
                  {!isPaid ? t("subscriptionPortal.noReceiptsFree") : t("subscriptionPortal.noReceiptsPaid")}
                </p>
              </div>
            )}
            {isPaid && !adminGranted && (
              <button onClick={openStripePortal} disabled={portalLoading} style={{ width: "100%", marginTop: 6, padding: "12px 0", background: "transparent", border: "none", color: "rgba(255,255,255,0.3)", fontSize: 12, fontWeight: 600, cursor: "pointer", textDecoration: "underline" }}>
                {portalLoading ? t("subscriptionPortal.openingPortal") : t("subscriptionPortal.viewInStripe")}
              </button>
            )}
          </div>
        )}
...
// ─── Plan Card ────────────────────────────────────────────────────────────────
function PlanCard({ config, isUpgrade, currentTier, onOpenPortal, t }: {
  config: any; isUpgrade: boolean; currentTier: string; onOpenPortal: () => void; t: (k: string, o?: any) => string;
}) {
    ...
    <div style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${config.glow}`, borderRadius: 22, padding: 20, boxShadow: `0 0 20px ${config.glow}` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <span style={{ background: isUpgrade ? "rgba(34,197,94,0.08)" : "rgba(255,255,255,0.04)", border: `1px solid ${isUpgrade ? "rgba(34,197,94,0.25)" : "rgba(255,255,255,0.1)"}`, borderRadius: 7, padding: "3px 9px", fontSize: 9, fontWeight: 800, color: isUpgrade ? "#22c55e" : "rgba(255,255,255,0.35)", letterSpacing: "0.08em" }}>
          {isUpgrade ? t("subscriptionPortal.tagUpgrade") : t("subscriptionPortal.tagDowngrade")}
        </span>
      </div>
      ...
      >
        {isInfinity ? t("subscriptionPortal.claimInfinity") : isUpgrade ? `${t("subscriptionPortal.upgradeTo")} ${config.label}` : `${t("subscriptionPortal.downgradeTo")} ${config.label}`}
      </button>
    </div>
  );
}
