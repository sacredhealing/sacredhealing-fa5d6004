...
export const AddToMeditationPlaylistSheet: React.FC<{
  meditationId: string | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}> = ({ meditationId, open, onOpenChange }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (!open || !user || !meditationId) return;
    (async () => {
      setLoading(true);
  ...
  const toggle = async (playlistId: string) => {
    ...
    if (isMember) {
      await supabase.from('meditation_playlist_items').delete().eq('playlist_id', playlistId).eq('meditation_id', meditationId);
      setMemberOf((s) => { const n = new Set(s); n.delete(playlistId); return n; });
      toast.success('Removed from playlist');
    } else {
      const order_index = await nextOrderIndex(playlistId);
      await supabase.from('meditation_playlist_items').insert({ playlist_id: playlistId, meditation_id: meditationId, order_index });
  ...
  const createAndAdd = async () => {
    if (!user || !meditationId || !newName.trim()) return;
    setCreating(true);
    ...
    await supabase.from('meditation_playlist_items').insert({ playlist_id: pl.id, meditation_id: meditationId, order_index: 0 });
    setPlaylists((p) => [pl, ...p]);
      ...
      <DialogContent className="max-w-md bg-[#0a0a0a] border-[#D4AF37]/25 p-5 max-h-[80vh] overflow-y-auto">
        ...
        {!loading && playlists.length === 0 && (
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', textAlign: 'center', padding: '20px 0' }}>
            You don't have any meditation playlists yet — create one above.
          </div>
        )}
...
export default AddToMeditationPlaylistSheet;
