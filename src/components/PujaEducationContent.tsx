...
import { PUJA_MODULES } from '@/data/pujaModuleContent';
...
const PujaEducationContent: React.FC<{ moduleKey: string; dbModuleId: string }> = ({ moduleKey, dbModuleId }) => {
  const num = parseInt(moduleKey.replace('m', ''), 10);
  const pModule = PUJA_MODULES.find((m) => parseInt(m.number, 10) === num);
  const [openIndex, setOpenIndex] = React.useState(0);
  const { rows: sectionRows, toggleSectionComplete, setSectionNotes } = useSectionProgress(dbModuleId, 'user_puja_education_section_progress');
  ...
  cards.push({
    sectionId: 'practice', title: `Practice — ${pModule.practice.name}`, meta: pModule.practice.duration, render: () => (
      <>
        <div style={{ marginBottom: 16 }}>
          {pModule.practice.elements.map((el, ei) => (
            <div key={ei} style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
              <span style={{ color: AMBER, flexShrink: 0 }}>◈</span>
              <p style={{ ...bodyStyle, fontSize: 15, margin: 0 }}>{el}</p>
            </div>
          ))}
        </div>
        <div style={{ background: 'rgba(217,119,6,0.06)', border: '1px solid rgba(217,119,6,0.25)', borderRadius: 18, padding: '16px 20px' }}>
          <p style={{ ...bodyStyle, fontStyle: 'italic', margin: 0 }}>{pModule.practice.sadhanaNote}</p>
        </div>
      </>
    ),
  });
  ...
  cards.push({
    sectionId: 'outcomes', title: 'Outcomes', render: () => (
      <ul style={{ margin: 0, paddingLeft: 18 }}>
        {pModule.outcomes.map((o, oi) => <li key={oi} style={{ ...bodyStyle, fontSize: 15.5, marginBottom: 8 }}>{o}</li>)}
      </ul>
    ),
  });
            ...
            <button type="button" onClick={() => setOpenIndex(isOpen ? -1 : i)} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  {c.meta && <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', marginTop: 2 }}>{c.meta}</div>}
                </div>
              </div>
...
export default PujaEducationContent;
