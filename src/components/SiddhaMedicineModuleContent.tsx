...
import { SIDDHA_MEDICINE_CURRICULUM } from '@/data/siddhaMedicineModuleContent';
        ...
        return (
          ...
          >
            ...
            >
              ...
              <ChevronDown
                size={16}
                style={{ flexShrink: 0, color: isOpen ? TEAL : 'rgba(255,255,255,0.3)', transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform .2s' }}
              />
            </button>
              ...
              <div style={{ padding: '0 20px 24px' }}>
                ...
                {c.teachings?.map((t, ti) => (
                  <div key={ti} style={{ marginBottom: 18 }}>
                    ...
                    <p style={bodyStyle}>{renderBody(t.body)}</p>
                  </div>
                ))}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 18, flexWrap: 'wrap', gap: 10 }}>
                  ...
                  <button
                    type="button"
                    disabled={i === smModule.lessons.length - 1}
                    onClick={() => setOpenIndex(Math.min(smModule.lessons.length - 1, i + 1))}
                    style={{
                      background: i === smModule.lessons.length - 1 ? 'transparent' : 'rgba(52,211,153,0.14)',
                      border: `1px solid ${i === smModule.lessons.length - 1 ? 'rgba(255,255,255,0.1)' : 'rgba(52,211,153,0.4)'}`,
                      color: i === smModule.lessons.length - 1 ? 'rgba(255,255,255,0.25)' : TEAL,
                      padding: '9px 16px', borderRadius: 999, fontSize: 10.5, fontWeight: 700,
                      cursor: i === smModule.lessons.length - 1 ? 'default' : 'pointer',
                    }}
                  >
                    {i === smModule.lessons.length - 1 ? 'Last lesson' : 'Next lesson →'}
                  </button>
                </div>
              </div>
            )}
