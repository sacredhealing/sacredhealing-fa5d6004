...
export interface SacredGeometryCourseRow {
  id: string;
...
export interface SacredGeometryProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface SacredGeometryStats {
  totalModules: number;
...
export function useSacredGeometryProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<SacredGeometryCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, SacredGeometryProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('sacred_geometry_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as SacredGeometryCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_sacred_geometry_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, SacredGeometryProgressSnapshot> = {};
        (progRows || []).forEach((row: SacredGeometryProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_sacred_geometry_progress').upsert(
  ...
  const stats = useMemo((): SacredGeometryStats => ({
