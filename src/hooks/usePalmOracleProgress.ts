...
export interface PalmOracleCourseRow {
  id: string;
...
export interface PalmOracleProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface PalmOracleStats {
  totalModules: number;
...
export function usePalmOracleProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<PalmOracleCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, PalmOracleProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('palm_oracle_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as PalmOracleCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_palm_oracle_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, PalmOracleProgressSnapshot> = {};
        (progRows || []).forEach((row: PalmOracleProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_palm_oracle_progress').upsert(
  ...
  const stats = useMemo((): PalmOracleStats => ({
