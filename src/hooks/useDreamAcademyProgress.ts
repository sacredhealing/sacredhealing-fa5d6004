...
export interface DreamCourseRow {
  id: string;
...
export interface DreamProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export function useDreamAcademyProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<DreamCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, DreamProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('dream_academy_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as DreamCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_dream_academy_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, DreamProgressSnapshot> = {};
        (progRows || []).forEach((row: DreamProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_dream_academy_progress').upsert(
  ...
  const stats = useMemo((): DreamStats => ({
