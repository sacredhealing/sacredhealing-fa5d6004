...
export interface NarasimhaCourseRow {
  id: string;
...
export interface NarasimhaProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface NarasimhaStats {
  totalModules: number;
...
export function useNarasimhaProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<NarasimhaCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, NarasimhaProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('narasimha_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as NarasimhaCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_narasimha_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, NarasimhaProgressSnapshot> = {};
        (progRows || []).forEach((row: NarasimhaProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_narasimha_progress').upsert(
  ...
  const stats = useMemo((): NarasimhaStats => ({
