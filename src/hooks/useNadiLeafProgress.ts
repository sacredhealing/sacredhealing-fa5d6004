...
export interface NadiLeafCourseRow {
  id: string;
...
export interface NadiLeafProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface NadiLeafStats {
  totalModules: number;
...
export function useNadiLeafProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<NadiLeafCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, NadiLeafProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('nadi_leaf_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as NadiLeafCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_nadi_leaf_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, NadiLeafProgressSnapshot> = {};
        (progRows || []).forEach((row: NadiLeafProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_nadi_leaf_progress').upsert(
  ...
  const stats = useMemo((): NadiLeafStats => ({
