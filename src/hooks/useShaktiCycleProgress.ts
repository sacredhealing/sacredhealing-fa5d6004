...
export interface ShaktiCourseRow {
  id: string;
...
export interface ShaktiProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export function useShaktiCycleProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<ShaktiCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, ShaktiProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('shakti_cycle_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as ShaktiCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_shakti_cycle_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, ShaktiProgressSnapshot> = {};
        (progRows || []).forEach((row: ShaktiProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_shakti_cycle_progress').upsert(
  ...
  const stats = useMemo((): ShaktiStats => ({
