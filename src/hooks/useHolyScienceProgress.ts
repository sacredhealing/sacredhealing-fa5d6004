...
export interface HolyScienceCourseRow {
  id: string;
...
export interface HolyScienceProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface HolyScienceStats {
  totalModules: number;
...
export function useHolyScienceProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<HolyScienceCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, HolyScienceProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('holy_science_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as HolyScienceCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_holy_science_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, HolyScienceProgressSnapshot> = {};
        (progRows || []).forEach((row: HolyScienceProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_holy_science_progress').upsert(
  ...
  const stats = useMemo((): HolyScienceStats => ({
