...
export interface HanumanCourseRow {
  id: string;
...
export interface HanumanProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export function useHanumanCodexProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<HanumanCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, HanumanProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('hanuman_codex_sections')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as HanumanCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_hanuman_codex_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, HanumanProgressSnapshot> = {};
        (progRows || []).forEach((row: HanumanProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_hanuman_codex_progress').upsert(
  ...
  const stats = useMemo((): HanumanStats => ({
