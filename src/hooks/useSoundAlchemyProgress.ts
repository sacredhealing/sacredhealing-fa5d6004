...
export interface SoundAlchemyCourseRow {
  id: string;
...
export interface SoundAlchemyProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface SoundAlchemyStats {
  totalModules: number;
...
export function useSoundAlchemyProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<SoundAlchemyCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, SoundAlchemyProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('sound_alchemy_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as SoundAlchemyCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_sound_alchemy_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, SoundAlchemyProgressSnapshot> = {};
        (progRows || []).forEach((row: SoundAlchemyProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_sound_alchemy_progress').upsert(
  ...
  const stats = useMemo((): SoundAlchemyStats => ({
