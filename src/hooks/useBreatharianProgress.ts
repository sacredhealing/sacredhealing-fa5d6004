...
export interface BreatharianCourseRow {
  id: string;
...
export interface BreatharianProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface BreatharianStats {
  totalModules: number;
...
const COURSE_SELECT = 'id, module_number, module_key, title, tier_required, is_published';
const PROGRESS_SELECT = 'module_id, completed, bookmarked, last_accessed_at';

export function useBreatharianProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<BreatharianCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, BreatharianProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('breatharian_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as BreatharianCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_breatharian_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, BreatharianProgressSnapshot> = {};
        (progRows || []).forEach((row: BreatharianProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_breatharian_progress').upsert(
  ...
  const stats = useMemo((): BreatharianStats => ({
