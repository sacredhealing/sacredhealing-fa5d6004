...
export interface PujaCourseRow {
  id: string;
...
export interface PujaProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export function usePujaEducationProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<PujaCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, PujaProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('puja_education_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as PujaCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_puja_education_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, PujaProgressSnapshot> = {};
        (progRows || []).forEach((row: PujaProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_puja_education_progress').upsert(
