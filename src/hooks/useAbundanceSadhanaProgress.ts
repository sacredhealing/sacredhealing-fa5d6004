...
export interface AbundanceCourseRow {
  id: string;
...
export interface AbundanceProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface AbundanceStats {
  totalModules: number;
...
export function useAbundanceSadhanaProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<AbundanceCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, AbundanceProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('abundance_sadhana_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as AbundanceCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_abundance_sadhana_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, AbundanceProgressSnapshot> = {};
        (progRows || []).forEach((row: AbundanceProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_abundance_sadhana_progress').upsert(
  ...
  const stats = useMemo((): AbundanceStats => ({
