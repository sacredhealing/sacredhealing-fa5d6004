...
export interface PranayamaCourseRow {
  id: string;
...
export interface PranayamaProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface PranayamaStats {
  totalModules: number;
...
export function usePranayamaProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<PranayamaCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, PranayamaProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('pranayama_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as PranayamaCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_pranayama_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, PranayamaProgressSnapshot> = {};
        (progRows || []).forEach((row: PranayamaProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_pranayama_progress').upsert(
  ...
  const stats = useMemo((): PranayamaStats => ({
