...
export interface YoganandaCourseRow {
  id: string;
...
export interface YoganandaProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface YoganandaStats {
  totalModules: number;
...
export function useYoganandaCodexProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<YoganandaCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, YoganandaProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('yogananda_codex_chapters')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as YoganandaCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_yogananda_codex_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, YoganandaProgressSnapshot> = {};
        (progRows || []).forEach((row: YoganandaProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_yogananda_codex_progress').upsert(
  ...
  const stats = useMemo((): YoganandaStats => ({
