...
export interface YagnaCourseRow {
  id: string;
...
export interface YagnaProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export function useYagnaProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<YagnaCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, YagnaProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (cErr) throw cErr;
      setCourses((courseRows || []) as YagnaCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_yagna_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, YagnaProgressSnapshot> = {};
        (progRows || []).forEach((row: YagnaProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_yagna_progress').upsert(
  ...
  const stats = useMemo((): YagnaStats => ({
