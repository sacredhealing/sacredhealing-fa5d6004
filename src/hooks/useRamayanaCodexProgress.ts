...
export interface RamayanaCourseRow {
  id: string;
...
export interface RamayanaProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface RamayanaStats {
  totalModules: number;
...
export function useRamayanaCodexProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<RamayanaCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, RamayanaProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('ramayana_codex_kandas')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as RamayanaCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_ramayana_codex_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, RamayanaProgressSnapshot> = {};
        (progRows || []).forEach((row: RamayanaProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_ramayana_codex_progress').upsert(
  ...
  const stats = useMemo((): RamayanaStats => ({
