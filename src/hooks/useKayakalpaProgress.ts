...
export interface KayakalpaCourseRow {
  id: string;
...
export interface KayakalpaProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface KayakalpaStats {
  totalModules: number;
...
const COURSE_SELECT = 'id, module_number, title, subtitle, tier_required, is_published';
const PROGRESS_SELECT = 'module_id, completed, bookmarked, last_accessed_at';

export function useKayakalpaProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<KayakalpaCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, KayakalpaProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('kayakalpa_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as KayakalpaCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_kayakalpa_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, KayakalpaProgressSnapshot> = {};
        (progRows || []).forEach((row: KayakalpaProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_kayakalpa_progress').upsert(
  ...
  const stats = useMemo((): KayakalpaStats => ({
