...
export interface BrahmacharyaCourseRow {
  id: string;
...
export interface BrahmacharyaProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface BrahmacharyaStats {
  totalModules: number;
...
export function useBrahmacharyaProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<BrahmacharyaCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, BrahmacharyaProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('brahmacharya_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as BrahmacharyaCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_brahmacharya_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, BrahmacharyaProgressSnapshot> = {};
        (progRows || []).forEach((row: BrahmacharyaProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_brahmacharya_progress').upsert(
  ...
  const stats = useMemo((): BrahmacharyaStats => ({
