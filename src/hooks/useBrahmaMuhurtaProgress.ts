...
export interface BrahmaMuhurtaCourseRow {
  id: string;
...
export interface BrahmaMuhurtaProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface BrahmaMuhurtaStats {
  totalModules: number;
...
export function useBrahmaMuhurtaProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<BrahmaMuhurtaCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, BrahmaMuhurtaProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('brahma_muhurta_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as BrahmaMuhurtaCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_brahma_muhurta_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, BrahmaMuhurtaProgressSnapshot> = {};
        (progRows || []).forEach((row: BrahmaMuhurtaProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_brahma_muhurta_progress').upsert(
  ...
  const stats = useMemo((): BrahmaMuhurtaStats => ({
