...
export interface PractitionerCertCourseRow {
  id: string;
...
export interface PractitionerCertProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export interface PractitionerCertStats {
  totalModules: number;
...
export function usePractitionerCertProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<PractitionerCertCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, PractitionerCertProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('practitioner_cert_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as PractitionerCertCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_practitioner_cert_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, PractitionerCertProgressSnapshot> = {};
        (progRows || []).forEach((row: PractitionerCertProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_practitioner_cert_progress').upsert(
  ...
  const stats = useMemo((): PractitionerCertStats => ({
