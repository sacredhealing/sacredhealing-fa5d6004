...
export interface MediumshipCourseRow {
  id: string;
...
export interface MediumshipProgressSnapshot {
  module_id: string;
  completed: boolean;
  bookmarked: boolean;
...
export function useMediumshipAcademyProgress(enabled = true) {
  const { user } = useAuth();
  const [courses, setCourses] = useState<MediumshipCourseRow[]>([]);
  const [progressByModuleId, setProgressByModuleId] = useState<Record<string, MediumshipProgressSnapshot>>({});
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const { data: courseRows, error: cErr } = await supabase
        .from('mediumship_academy_courses')
        .select(COURSE_SELECT)
        .order('module_number', { ascending: true });
      if (cErr) throw cErr;
      setCourses((courseRows || []) as MediumshipCourseRow[]);

      if (user?.id) {
        const { data: progRows, error: pErr } = await supabase
          .from('user_mediumship_academy_progress')
          .select(PROGRESS_SELECT)
          .eq('user_id', user.id);
        if (pErr) throw pErr;
        const map: Record<string, MediumshipProgressSnapshot> = {};
        (progRows || []).forEach((row: MediumshipProgressSnapshot) => {
  ...
  const markComplete = useCallback(
    async (moduleId: string) => {
      ...
      const { error: upErr } = await supabase.from('user_mediumship_academy_progress').upsert(
  ...
  const stats = useMemo((): MediumshipStats => ({
