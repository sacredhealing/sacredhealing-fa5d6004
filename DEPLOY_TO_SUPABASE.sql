...
DROP POLICY IF EXISTS "Users can view own usage" ON public.creative_soul_usage;
DROP POLICY IF EXISTS "Service can manage usage" ON public.creative_soul_usage;
...
-- Drop existing policies if they exist (to avoid conflicts)
DROP POLICY IF EXISTS "Users can view own entitlements" ON public.creative_soul_entitlements;
DROP POLICY IF EXISTS "Service can manage entitlements" ON public.creative_soul_entitlements;
...
DROP TRIGGER IF EXISTS update_creative_soul_usage_updated_at ON public.creative_soul_usage;
DROP TRIGGER IF EXISTS update_creative_soul_entitlements_updated_at ON public.creative_soul_entitlements;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'creative_soul_usage' AND table_schema = 'public') THEN
    RAISE NOTICE '✅ creative_soul_usage table created successfully';
  ELSE
    RAISE EXCEPTION '❌ creative_soul_usage table was not created';
  END IF;

  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'creative_soul_entitlements' AND table_schema = 'public') THEN
    RAISE NOTICE '✅ creative_soul_entitlements table created successfully';
  ELSE
    RAISE EXCEPTION '❌ creative_soul_entitlements table was not created';
  END IF;
END $$;
