#!/usr/bin/env node
/**
 * SQI-2050 CLAWBOT v7 — Production Ready
 * Detects real on-chain whale buys, mirrors valid entry-zone trades
 */
const SUPABASE_URL = process.env.SUPABASE_URL;
const SUPABASE_KEY = process.env.SUPABASE_SERVICE_KEY;
const POLYGON_RPC  = process.env.POLYGON_RPC_URL;
const PAPER_MODE   = (process.env.PAPER_MODE ?? 'true') === 'true';
const MIN_USDC     = parseFloat(process.env.MIN_TRADE_USDC || '100'); // $100 min
const MAX_USDC     = 10_000_000;
const DECIMALS     = 1_000_000;
const MIN_PRICE    = 0.06;  // don't mirror near-resolved (< 6%)
const MAX_PRICE    = 0.94;  // don't mirror near-resolved (> 94%)
...
function decodeLog(log) {
  ...
  if (!tokenId || tokenId.length < 5) return null; // real tokenIds are long
...
async function scanContract(contract, fromBlock, toBlock) {
  const trades = []; let total = 0;
  for (let b = fromBlock; b <= toBlock; b += 9) {
    const end = Math.min(b + 8, toBlock);
    try {
      const logs = await rpc('eth_getLogs', [{
        fromBlock: '0x' + b.toString(16), toBlock: '0x' + end.toString(16), address: contract,
      }]);
      total += logs.length;
      for (const log of logs) { const t = decodeLog(log); if (t) trades.push(t); }
    } catch {}
...
async function getMarket(tokenId) {
  try {
    ...
    return {
      ...
      price:   parseFloat(idx >= 0 ? prices[idx] : 0.5),
...
async function main() {
  console.log('══════════════════════════════════════════════════════');
  console.log('  SQI-2050 🦈 CLAWBOT v7 — Production');
  console.log(`  ${new Date().toISOString()} | ${PAPER_MODE ? 'PAPER' : 'LIVE'}`);
  console.log(`  Min: $${MIN_USDC} | Entry zone: ${MIN_PRICE*100}%–${MAX_PRICE*100}%`);
  console.log('══════════════════════════════════════════════════════');
  ...
  const seenRows = await dbGet('polymarket_seen_trades', 'limit=3000&order=detected_at.desc');
  ...
  for (const c of CONTRACTS) {
    ...
    console.log(`  ${c.slice(0,12)}: ${total} logs → ${trades.length} whale trades`);
  ...
  for (const trade of newTrades) {
    // Skip duplicate txHash for same market
    ...
    // Only mirror mid-range entries (entry zone, not exits)
    if (price < MIN_PRICE || price > MAX_PRICE) continue;
    if (balance < 0.15) { console.log('\nBalance too low, stopping.'); break; }

    if (mirrored >= 3) break; // max 3 mirrors per run

    const [whale] = await dbGet('polymarket_whales', `address=eq.${trade.maker}&limit=1`);
    if (!whale) await dbIns('polymarket_whales', {
    ...
    const ok = await dbIns('polymarket_trades', {
      ...
      strategy: 'clawbot_v7', is_paper: true, status: 'open',
    });
    ...
    if (ok) {
      ...
      console.log(`   Whale $${trade.usdcSize.toFixed(0)} → Mirror $${size} | ${outcome} @ ${(price*100).toFixed(1)}%`);
      console.log(`   ${question.slice(0,80)}`);
    }
    await new Promise(r => setTimeout(r, 400));
  }

  console.log(`\n━━━ CLAWBOT v7: ${mirrored} mirrored | Bal $${balance.toFixed(4)} ━━━`);
}
