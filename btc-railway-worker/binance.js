...
class BinanceClient {
  ...
  _Sign(q) {
    return crypto.createHmac("sha256", this.apiSecret).update(q).digest("hex");
  }
  async _privateRequest(method, endpoint, params = {}) {
    ...
    if (!res.ok) {
      ...
      throw new Error("Binance " + res.status + ": " + (e.msg || ""));
    }
    return res.json();
  }
  async getTicker() {
    ...
    return { last: d.price, price: d.price, bid: d.price, ask: d.price };
  }
  ...
  async getBalances() {
    if (this.paperMode)
      return [
        { asset: "USDT", free: "10.00", locked: "0" },
  ...
  async placeOrder(side, quantity, price = null) {
    ...
    const params = {
      ...
      quantity: qty,
      ...(price ? { price: price.toFixed(2), timeInForce: "IOC" } : {}),
    };
  ...
  subscribeMarketData(onPrice, onError) {
    ...
    ws.on("message", (raw) => {
      try {
        ...
        if (m.e === "trade")
          onPrice({ price: parseFloat(m.p), amount: parseFloat(m.q), side: m.m ? "sell" : "buy", timestamp: m.T });
      } catch (e) {}
    });
    ws.on("error", (err) => {
    ...
    ws.on("close", () => {
      console.log("WS closed - reconnecting 5s...");
      setTimeout(() => this.subscribeMarketData(onPrice, onError), 5000);
    });
