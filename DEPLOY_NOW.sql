...
-- Copy the entire content below and paste into Supabase Dashboard → SQL Editor → Run
-- =====================================================
...
DROP POLICY IF EXISTS "Users can view their own entitlements" ON public.creative_soul_entitlements;
...
DROP POLICY IF EXISTS "Service role can manage wallet" ON public.user_wallet;
...
DROP POLICY IF EXISTS "Users can view their own coin awards" ON public.coin_awards;
CREATE POLICY "Users can view their own coin awards"
  ON public.coin_awards FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Service role can manage coin awards" ON public.coin_awards;
...
DROP POLICY IF EXISTS "Users can view their own attribution" ON public.affiliate_attribution;
...
DROP POLICY IF EXISTS "Users can view their own affiliate events" ON public.affiliate_events;
...
DROP TRIGGER IF EXISTS update_user_wallet_updated_at ON public.user_wallet;
